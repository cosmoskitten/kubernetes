...
function kube::build::prepare_docker_machine() {
  ...
  local available_memory=$(sysctl -n hw.memsize 2>/dev/null)
  local virtualbox_memory=$(( (${available_memory} * 1024) / (1073741824 * 2) ))
  ...
  docker-machine inspect "${DOCKER_MACHINE_NAME}" &> /dev/null || {
    ...
    docker-machine create --driver ${DOCKER_MACHINE_DRIVER} \
      ...
      kube::log::error "docker-machine create -d ${DOCKER_MACHINE_DRIVER} --virtualbox-memory ${virtualbox_memory} ${DOCKER_MACHINE_NAME}"
      return 1
    }
  }
