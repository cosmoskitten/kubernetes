...
func createKubeProxyAddon(configMapBytes, daemonSetbytes []byte, client clientset.Interface) error {
	...
	if err := kuberuntime.DecodeInto(globalscheme.Codecs.UniversalDecoder(), configMapBytes, kubeproxyConfigMap); err != nil {
	...
	if err := kuberuntime.DecodeInto(globalscheme.Codecs.UniversalDecoder(), daemonSetbytes, kubeproxyDaemonSet); err != nil {
