...
func CreateNewToken(client clientset.Interface, token string, tokenDuration time.Duration, usages []string, groups []string, description string) error {
	return UpdateOrCreateToken(client, token, true, tokenDuration, usages, groups, description)
}

// UpdateOrCreateToken attempts to update a token with the given ID, or create if it does not already exist.
func UpdateOrCreateToken(client clientset.Interface, token string, failIfExists bool, tokenDuration time.Duration, usages []string, groups []string, description string) error {
	...
	for i := 0; i < tokenCreateRetries; i++ {
		...
		if err == nil {
			...
			secret.Data = encodeTokenSecretData(tokenID, tokenSecret, tokenDuration, usages, groups, description)
		...
		if apierrors.IsNotFound(err) {
			secret = &v1.Secret{
				...
				Data: encodeTokenSecretData(tokenID, tokenSecret, tokenDuration, usages, groups, description),
			}
...
func encodeTokenSecretData(tokenID, tokenSecret string, duration time.Duration, usages []string, groups []string, description string) map[string][]byte {
	data := map[string][]byte{
		bootstrapapi.BootstrapTokenIDKey:     []byte(tokenID),
		bootstrapapi.BootstrapTokenSecretKey: []byte(tokenSecret),
		bootstrapapi.BootstrapTokenGroupsKey: []byte(strings.Join(groups, ",")),
	}
