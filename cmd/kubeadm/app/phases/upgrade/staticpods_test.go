...
const (
	...
	waitForComponentReady = "wait-for-component-ready"
...
// fakeStaticPodWaiter is a fake StaticPodWaiter that returns errors it was initialized with
type fakeStaticPodWaiter struct {
	errsToReturn map[string]error
}

func NewFakeStaticPodWaiter(errsToReturn map[string]error) StaticPodWaiter {
	return &fakeStaticPodWaiter{
		errsToReturn: errsToReturn,
	}
}

func (fsp *fakeStaticPodWaiter) WaitForHashes(_ string) (map[string]string, error) {
	return map[string]string{}, fsp.errsToReturn[waitForHashes]
}

func (fsp *fakeStaticPodWaiter) WaitForHashChange(_, _, _ string) error {
	return fsp.errsToReturn[waitForHashChange]
}

func (fsp *fakeStaticPodWaiter) WaitForComponentReady(_ string) error {
	return fsp.errsToReturn[waitForComponentReady]
}
...
func NewFakeStaticPodPathManager(moveFileFunc func(string, string) error) (StaticPodPathManager, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("couldn't create a temporary directory for the upgrade: %v")
	}
...
func TestUpgradeStaticPodControlPlane(t *testing.T) {
	tests := []struct {
		...
		{ // any wait error should result in a rollback and an abort
			waitErrsToReturn: map[string]error{
				waitForHashes:         nil,
				waitForHashChange:     nil,
				waitForComponentReady: fmt.Errorf("boo! failed!"),
			},
			moveFileFunc: func(oldPath, newPath string) error {
				return os.Rename(oldPath, newPath)
			},
	...
	for _, rt := range tests {
		...
		actualErr := UpgradeStaticPodControlPlane(waiter, pathMgr, newcfg)
