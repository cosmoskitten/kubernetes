...
func UpgradeStaticPodControlPlane(client clientset.Interface, cfg *kubeadmapi.MasterConfiguration, newK8sVersion *version.Version) error {
	...
	// This string-string map stores the component name and backup filepath.
	...
	beforePodHashMap, err := kubeadmutil.WaitForStaticPodControlPlaneHashes(client, waitForComponentReadyTimeout, cfg.NodeName)
	if err != nil {
		return err
	}

	// Write the updated static Pod manifests into the temporary directory
	err = controlplane.WriteStaticPodManifests(cfg, newK8sVersion, tmpdir)
	...
	for _, component := range constants.MasterComponents {
		...
		fmt.Printf("[upgrade/staticpods] Moved upgraded manifest to %q and backuped old manifest to %q\n", currentManifestPath, backupManifestPath)
		...
		if err := kubeadmutil.WaitForStaticPodControlPlaneHashChange(client, waitForComponentReadyTimeout, cfg.NodeName, component, beforePodHashMap[component]); err != nil {
			// We need to rollback the old version
			return rollbackOldManifests(recoverManifests, err)
		}

		// Wait for the static pod component to come up and register itself as a mirror pod
		if err := kubeadmutil.WaitForPodsWithLabel(client, waitForComponentReadyTimeout, os.Stdout, "component="+component); err != nil {
			// We need to rollback the old version
			return rollbackOldManifests(recoverManifests, err)
		}
...
func rollbackOldManifests(oldManifests map[string]string, origErr error) error {
	...
	for component, backupPath := range oldManifests {
		// Where we should put back the backuped manifest
