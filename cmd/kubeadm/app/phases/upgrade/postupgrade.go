...
import (
	...
	//"k8s.io/kubernetes/cmd/kubeadm/app/phases/token"
	"k8s.io/kubernetes/cmd/kubeadm/app/phases/addons"
	"k8s.io/kubernetes/cmd/kubeadm/app/phases/apiconfig"
	"k8s.io/kubernetes/cmd/kubeadm/app/phases/saveconfig"
...
func PerformPostUpgradeTasks(client clientset.Interface, cfg *kubeadmapi.MasterConfiguration, k8sVersion *version.Version) error {
	...
	// if err := token.CreateBootstrapConfigMapIfNotExists(client, adminKubeConfigPath); err != nil {
	// 	return err
	// }
	...
	if err := saveconfig.SaveConfiguration(cfg, client); err != nil {
		return err
	}
	return nil
}
