...
// GetClusterVersion gets a fake API server version
func (f *fakeVersionGetter) GetClusterVersion() (string, *versionutil.Version, error) {
	return f.clusterVersion, versionutil.MustParseSemantic(f.clusterVersion), nil
}

// GetKubeadmVersion gets a fake kubeadm version
func (f *fakeVersionGetter) GetKubeadmVersion() (string, *versionutil.Version, error) {
	return f.kubeadmVersion, versionutil.MustParseSemantic(f.kubeadmVersion), nil
}

// GetLatestKubeVersion gets fake latest versions from CI
func (f *fakeVersionGetter) GetLatestKubeVersion(ciVersionLabel, _ string) (string, *versionutil.Version, error) {
...
func (f *fakeVersionGetter) GetKubeletVersions() (map[string]int32, error) {
