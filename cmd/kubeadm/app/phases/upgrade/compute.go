...
func GetAvailableUpgrades(versionGetterImpl versionGetter, experimentalUpgradesAllowed, rcUpgradesAllowed bool) ([]Upgrade, error) {
	...
	clusterVersionStr, clusterVersion, err := versionGetterImpl.GetClusterVersion()
	if err != nil {
		return nil, err
	}

	// Get current kubeadm CLI version
	kubeadmVersionStr, kubeadmVersion, err := versionGetterImpl.GetKubeadmVersion()
	if err != nil {
		return nil, err
	}

	// Get and output the current latest stable version
	stableVersionStr, stableVersion, err := versionGetterImpl.GetLatestKubeVersion("stable", "stable version")
	if err != nil {
		return nil, err
	}

	// Get the kubelet versions in the cluster
	kubeletVersions, err := versionGetterImpl.GetKubeletVersions()
	if err != nil {
		return nil, err
	}

	// Construct a descriptor for the current state of the world
	beforeState := StateBeforeUpgrade{
		KubeVersion:     clusterVersionStr,
		DNSVersion:      constants.GetKubeDNSVersion(clusterVersion),
	...
	// In the case an user is trying to upgrade from, let's say, v1.8.0-beta.2 to v1.8.0-rc.1 (given we support such upgrades experimentally)
	...
	if patchVersionBranchExists(clusterVersion, stableVersion) {
		...
		patchVersionStr, patchVersion, err := versionGetterImpl.GetLatestKubeVersion(versionLabel, description)
		if err != nil {
			return nil, err
		}
		...
		if patchUpgradePossible(clusterVersion, patchVersion) {
			...
			upgrades = append(upgrades, Upgrade{
				...
				After: StateAfterUpgrade{
					...
					DNSVersion:     constants.GetKubeDNSVersion(patchVersion),
					KubeadmVersion: newKubeadmVer,
					KubeletVersion: patchVersionStr,
				},
			})
		}
	}
	...
	if canDoMinorUpgrade {
		upgrades = append(upgrades, Upgrade{
			...
			After: StateAfterUpgrade{
				...
				DNSVersion:     constants.GetKubeDNSVersion(stableVersion),
				KubeadmVersion: stableVersionStr,
				KubeletVersion: stableVersionStr,
			},
		})
	}
	...
	if experimentalUpgradesAllowed || rcUpgradesAllowed {
		...
		latestVersionStr, latestVersion, err := versionGetterImpl.GetLatestKubeVersion("latest", "experimental version")
		if err != nil {
			return nil, err
		}
		...
		previousBranchLatestVersionStr, previousBranchLatestVersion, err := versionGetterImpl.GetLatestKubeVersion(previousBranch, "")
		if err != nil {
			return nil, err
		}
		...
		if rcUpgradesAllowed && rcUpgradePossible(clusterVersion, previousBranchLatestVersion) {
			upgrades = append(upgrades, Upgrade{
				...
				After: StateAfterUpgrade{
					...
					DNSVersion:     constants.GetKubeDNSVersion(previousBranchLatestVersion),
					KubeadmVersion: previousBranchLatestVersionStr,
					KubeletVersion: previousBranchLatestVersionStr,
				},
			})
		}
		...
		if experimentalUpgradesAllowed && clusterVersion.LessThan(latestVersion) {
			...
			unstableKubeDNSVersion := constants.GetKubeDNSVersion(latestVersion)
			...
			if latestVersion.PreRelease() == "alpha.0" {
				...
				unstableKubeDNSVersion = constants.GetKubeDNSVersion(previousBranchLatestVersion)
			}
			...
			upgrades = append(upgrades, Upgrade{
				...
				After: StateAfterUpgrade{
					...
					KubeletVersion: unstableKubeVersion,
				},
			})
		}
	}
