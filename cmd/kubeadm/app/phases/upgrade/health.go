...
func VerifyClusterHealth(client clientset.Interface) error {
	...
	err := runHealthChecks(client, defaultHealthChecks)
	if err != nil {
		return err
	}
	...
	if IsControlPlaneSelfHosted(client) {
		...
		err = runHealthChecks(client, []healthCheck{
			{
...
func runHealthChecks(client clientset.Interface, healthChecks []healthCheck) error {
	...
	for _, check := range healthChecks {
		...
		if err != nil {
			...
			return fmt.Errorf("[upgrade/health] The cluster is not in an upgradeable state due to: %v", err)
		}
