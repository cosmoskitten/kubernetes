...
func EnforceVersionPolicies(versionGetter versionGetter, newK8sVersionStr string, newK8sVersion *version.Version, allowExperimentalUpgrades, allowRCUpgrades bool) *VersionSkewPolicyErrors {
	...
	newK8sVersionMinor := newK8sVersion.Components()[1]

	clusterVersionStr, clusterVersion, err := versionGetter.GetClusterVersion()
	if err != nil {
	...
	kubeadmVersionStr, kubeadmVersion, err := versionGetter.GetKubeadmVersion()
	if err != nil {
	...
	kubeletVersions, err := versionGetter.GetKubeletVersions()
	if err != nil {
	...
	if clusterVersion.AtLeast(newK8sVersion) {
		...
		clusterMinor := clusterVersion.Components()[1]
		if newK8sVersionMinor > clusterMinor+MaximumAllowedMinorVersionUpgradeSkew {
	...
	if kubeadmVersion.LessThan(newK8sVersion) {
		kubeadmMinor := kubeadmVersion.Components()[1]

		if newK8sVersionMinor > kubeadmMinor {
			// This is totally unsupported; kubeadm has no idea how it should handle a newer minor release than itself
			skewErrors.Mandatory = append(skewErrors.Mandatory, fmt.Errorf("Specified version to upgrade to %q is one minor release higher than the kubeadm minor release (%d > %d). Such an upgrade is not supported", newK8sVersionStr, newK8sVersionMinor, kubeadmMinor))
		} else {
	...
	// Check for nil here since this is the only case where kubeletVersions can be nil; if GetKubeletVersions() returned an error
...
func detectTooOldKubelets(newK8sVersion *version.Version, kubeletVersions map[string]int32) error {
	...
	for versionStr := range kubeletVersions {
		...
		newMinor := newK8sVersion.Components()[1]
		kubeletMinor := kubeletVersion.Components()[1]

		if newMinor > kubeletMinor+MaximumAllowedMinorVersionKubeletSkew {
