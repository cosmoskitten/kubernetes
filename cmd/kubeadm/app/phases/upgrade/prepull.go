...
func (d *DaemonSetPrepuller) CreateFunc(component string) {
	ds := buildPrePullDaemonSet(d.cfg, component)
...
func (d *DaemonSetPrepuller) WaitFunc(component string) {
	...
	kubeadmutil.WaitForPodsWithLabel(d.client, 0, ioutil.Discard, "k8s-app=upgrade-prepull-"+component)
}
...
// PrepullImagesInParallell creates DaemonSets synchronously but waits in parallell for the images to pull
func PrepullImagesInParallell(kubePrepuller prepuller, timeout time.Duration) error {
...
func buildPrePullDaemonSet(cfg *kubeadmapi.MasterConfiguration, component string) *extensions.DaemonSet {
			...
			Template: v1.PodTemplateSpec{
				...
				Spec: v1.PodSpec{
					Containers: []v1.Container{
						{
							Name:    component,
							Image:   images.GetCoreImage(component, cfg.ImageRepository, cfg.KubernetesVersion, cfg.UnifiedControlPlaneImage),
