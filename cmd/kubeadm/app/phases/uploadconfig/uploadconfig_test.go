	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.verifyResult {
				...
				if err := runtime.DecodeInto(globalscheme.Codecs.UniversalDecoder(), []byte(configData), decodedExtCfg); err != nil {
					t.Errorf("unable to decode config from bytes: %v", err)
				}
				// Default and convert to the internal version
				globalscheme.Scheme.Default(decodedExtCfg)
				globalscheme.Scheme.Convert(decodedExtCfg, decodedCfg, nil)
