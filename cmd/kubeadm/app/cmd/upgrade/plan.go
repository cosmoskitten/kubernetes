...
func RunPlan(parentFlags *cmdUpgradeFlags) error {
	...
	// Start with the basics, verify that the cluster is healthy, build a client and a versionGetter. Always do this non-interactively for plan
	upgradeVars, err := EnforceRequirements(parentFlags.kubeConfigPath, parentFlags.cfgPath, parentFlags.printConfig, true)
	if err != nil {
		return err
	}
