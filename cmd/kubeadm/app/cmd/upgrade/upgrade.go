...
func NewCmdUpgrade(out io.Writer) *cobra.Command {
	var kubeConfigPath, cfgPath string
	...
	cmd.PersistentFlags().StringVar(&kubeConfigPath, "kubeconfig", "/etc/kubernetes/admin.conf", "The KubeConfig file to use for talking to the cluster")
	cmd.PersistentFlags().StringVar(&cfgPath, "config", "", "Path to kubeadm config file (WARNING: Usage of a configuration file is experimental)")

	cmd.AddCommand(NewCmdApply())
	cmd.AddCommand(NewCmdPlan())
