...
var (
	...
	kubeletFailTempl = template.Must(template.New("init").Parse(dedent.Dedent(`
		...
		If you're on a systemd-powered system, you can try to troubleshoot the error with the following commands:
...
func AddInitConfigFlags(flagSet *flag.FlagSet, cfg *kubeadmapiext.MasterConfiguration, featureGatesString *string) {
	flagSet.StringVar(
		...
		"The IP address the API Server will advertise it's listening on. Address '0.0.0.0' is reserved for the default network interface.",
	)
	flagSet.Int32Var(
	...
	flagSet.StringSliceVar(
		...
		`Optional extra "altnames" to use for the API Server serving certificate. Can be both IP addresses and DNS names.`,
	)
	flagSet.StringVar(
