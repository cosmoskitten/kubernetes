...
func NewCmdConfig(out io.Writer) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: fmt.Sprintf(dedent.Dedent(`
			...
			cluster. kubeadm CLI v1.8.0+ automatically creates this ConfigMap with a config used with 'kubeadm init', but if you
			initialized your cluster using kubeadm v1.7.x or lower, you must use the 'config upload' command to create this
			ConfigMap. This is required so that 'kubeadm upgrade' can to configure your upgraded cluster correctly.
