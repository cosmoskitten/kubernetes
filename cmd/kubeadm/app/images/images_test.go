...
func TestGetCoreImage(t *testing.T) {
	var tests = []struct {
		...
		},
		{
			...
			repo:     gcrPrefix,
			expected: fmt.Sprintf("%s/%s-%s:%s", gcrPrefix, "etcd", runtime.GOARCH, etcdVersion),
		},
		{
			image:    constants.KubeAPIServerImage,
			repo:     gcrPrefix,
			version:  testversion,
			expected: fmt.Sprintf("%s/%s-%s:%s", gcrPrefix, "kube-apiserver", runtime.GOARCH, expected),
		},
		{
			image:    constants.KubeControllerManagerImage,
			repo:     gcrPrefix,
			version:  testversion,
			expected: fmt.Sprintf("%s/%s-%s:%s", gcrPrefix, "kube-controller-manager", runtime.GOARCH, expected),
		},
		{
			image:    constants.KubeSchedulerImage,
			repo:     gcrPrefix,
			version:  testversion,
