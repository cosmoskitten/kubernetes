...
func StartTestServer(t *testing.T, etcd EtcdProvider) (result *restclient.Config, opts *options.ServerRunOptions, tearDownForCaller TearDownFunc, err error) {
	var tmpDir string
	...
	etcdServer, storageConfig := etcd(t)
...
func StartTestServerOrDie(t *testing.T, etcd EtcdProvider) (*restclient.Config, *options.ServerRunOptions, TearDownFunc) {
	...
	for retry := 0; retry < 5 && !t.Failed(); retry++ {
		...
		config, opts, td, err := StartTestServer(t, etcd)
		if err == nil {
