...
function kube::godep_save::cleanup() {
    if [[ -d "${TMPDIR}/vendor" ]]; then
        kube::log::error "${TMPDIR}/vendor exists, restoring it"
        rm -rf vendor
        mv "${TMPDIR}/vendor" vendor
    fi
    if [[ -d "${TMPDIR}/Godeps" ]]; then
        kube::log::error "${TMPDIR}/Godeps exists, restoring it"
        rm -rf Godeps
        mv "${TMPDIR}/Godeps" Godeps
    fi
}
