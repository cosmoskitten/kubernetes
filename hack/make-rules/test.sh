...
verifyAndSuggestPackagePath() {
  ...
  if ! [ -d "$specified_package_path" ]; then
    ...
    if [ -d "$alternative_package_path" ]; then
      kube::log::info "if you intended to test \"$alternative_package_path\", change
      \"$original_package_path\" to \"$suggestion_package_path\""
    fi
    exit 1
  fi
}
...
verifyPathsToPackagesUnderTest() {
  ...
  for package_path in "${packages_under_test[@]}"; do
    local local_package_path="$(pwd)/$package_path"
