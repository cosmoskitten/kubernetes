...
run_kubectl_alpha_apply_manifest_tests() {
  set -o nounset
  set -o errexit

  create_and_use_new_namespace
  kube::log::status "Testing kubectl alpha apply-manifest"
  ## kubectl apply should create the resource that doesn't exist yet
  # Pre-Condition: no deployment exists
  kube::test::get_object_assert deployments "{{range.items}}{{$id_field}}:{{end}}" ''
  ...
  # Post-Condition: env toDrop=drop doesn't exists.
  ! [[ "$(kubectl get deployments test-infra-mungebot -o yaml "${kube_flags[@]}" | grep "name: toDrop")" ]]
