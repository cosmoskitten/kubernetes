...
run_clusterroles_tests() {
  ...
  kubectl create "${kube_flags[@]}" clusterrolebinding mult-user --clusterrole=admin --user=user-1 --user=user-2
  kube::test::get_object_assert clusterrolebinding/mult-user "{{range.subjects}}{{.name}}:{{end}}" 'user-1:user-2:'
  ...
  kubectl create "${kube_flags[@]}" clusterrolebinding mult-group --clusterrole=admin --group=group-1 --group=group-2
  kube::test::get_object_assert clusterrolebinding/mult-group "{{range.subjects}}{{.name}}:{{end}}" 'group-1:group-2:'
