...
run_deployment_tests() {
  ...
  # Check deployment web has been deleted
  output_message=$(! kubectl get deployment web 2>&1 "${kube_flags[@]}")
  # Post-condition: Deployment web should error since it doesn't exist
...
run_rs_tests() {
  ...
  # Check rs nginx has been deleted
  output_message=$(! kubectl get rs nginx 2>&1 "${kube_flags[@]}")
  # Post-condition: Replicasets nginx should error since it doesn't exist
