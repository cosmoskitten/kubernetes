...
There is no support for schudling best-effort pods in scheduler yet though
having the metrics available in api server is consindered as a blocker for
extensing the scheduler functionality.
...
The desing of the pipeline for collecting application level metrics should
be revisited and it's no clear whether application level metrics should be
...
- ```kubectl top pod --selector=<label-query>``` - list all pods defined by the label query accompanied with their cpu/mem usage
...
- watch for metrics
- support for different window sizes and aggreation functions
