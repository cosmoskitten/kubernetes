...
#
# This script is intended to be used on an instance with no FlexVolume installed.
...
if [ -z ${MOUNTER_IMAGE} ]; then
  echo "No Container Registry mounter image is specified."
  echo
  usage
fi
...
generate_chroot_wrapper() {
  if [ ! -d ${MOUNTER_PATH}/flexvolume ]; then
    echo "Cannot find directory '/flexvolume' in the mount utility image."
    exit 1
  fi
  ...
  for driver_dir in ${MOUNTER_PATH}/flexvolume/*; do
    if [ -d "$driver_dir" ]; then
      ...
      # Assuming the driver is the only file in the directory.
...
if [ $? -eq 0 ]; then
  ...
  echo "Timed out after 1 minute waiting for kubelet restart."
fi
