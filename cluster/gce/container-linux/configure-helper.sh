...
function start-kube-addons {
  ...
  if [[ "${ENABLE_CLUSTER_MONITORING:-}" == "influxdb" ]] || \
    ...
    base_metrics_memory="${HEAPSTER_GCP_BASE_MEMORY:-175Mi}"
    base_eventer_memory="237Mi"
    base_metrics_cpu="${HEAPSTER_GCP_BASE_CPU:-100m}"
    nanny_memory="90Mi"
    local -r metrics_memory_per_node="${HEAPSTER_GCP_MEMORY_PER_NODE:-6}"
    local -r metrics_cpu_per_node="${HEAPSTER_GCP_CPU_PER_NODE:-0.7}"
    local -r eventer_memory_per_node="625"
