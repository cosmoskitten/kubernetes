	...
	if x == nil {
		...
		if false {
			...
			if yyr2 || yy2arr2 {
				z.EncSendContainerState(codecSelfer_containerArrayElem1234)
				if yyq2[2] {
					if x.IPBlockSelector == nil {
						r.EncodeNil()
					} else {
						x.IPBlockSelector.CodecEncodeSelf(e)
					}
				} else {
					r.EncodeNil()
				}
...
func (x *NetworkPolicyPeer) codecDecodeSelfFromMap(l int, d *codec1978.Decoder) {
	...
	for yyj3 := 0; ; yyj3++ {
		...
		case "ipBlock":
			if r.TryDecodeAsNil() {
				if x.IPBlockSelector != nil {
					x.IPBlockSelector = nil
				}
			} else {
				if x.IPBlockSelector == nil {
					x.IPBlockSelector = new(IPBlock)
				}
				x.IPBlockSelector.CodecDecodeSelf(d)
			}
		default:
			z.DecStructFieldNotFound(-1, yys3)
		} // end switch yys3
