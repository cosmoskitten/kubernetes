...
// Example:
//
// +genclient:method=Instantiate,verb=create,subresource=instantiate,input=DeploymentRequest,result=DeploymentConfig
//
// type DeploymentConfigs struct { ... }
//
// The 'method=Instantiate' is the name of the client function.
// The 'verb=create' here means the client function will use 'POST' action.
// Only a verb from 'SupportedVerbs' can be used as a prefix. The
// 'deleteCollection' and 'updateStatus' verbs are not supported.
// The 'subresource=instantiate' means we will use SubResource template to
// generate this client function.
...
func parseClientExtensions(tags map[string][]string) ([]extension, error) {
	...
	for name, values := range tags {
		...
		for _, value := range values {
			...
			// Other parts are additional configuration (verb=TYPE is required)
