...
type EventCorrelatorOptions struct {
	...
	CacheSize int
	// The amount of events allowed per source and object
	SpamBurst int
...
func NewDefaultEventCorrelatorOptions() *EventCorrelatorOptions {
	return NewEventCorrelatorOptions(maxLruCacheEntries, defaultSpamBurst, defaultSpamQPS, defaultAggregateMaxEvents, defaultAggregateIntervalInSeconds)
}

func NewEventCorrelatorOptions(cacheSize, spamBurst int, spamQPS float32, aggregateMax, aggregateInterval int) *EventCorrelatorOptions {
	return &EventCorrelatorOptions{
		CacheSize:                  cacheSize,
		SpamBurst:                  spamBurst,
		SpamQPS:                    spamQPS,
		AggregateMaxEvents:         aggregateMax,
		AggregateIntervalInSeconds: aggregateInterval,
	}
}
