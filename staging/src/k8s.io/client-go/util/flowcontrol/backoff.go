...
// GetWithNbRetry the current backoff Duration and the number of previous retry
func (p *Backoff) GetWithNbRetry(id string) (time.Duration, int32) {
	p.Lock()
	defer p.Unlock()
	var delay time.Duration
	var nbRetry int32
...
func (p *Backoff) Next(id string, eventTime time.Time) {
	...
	if !ok || hasExpired(eventTime, entry.lastUpdate, p.maxDuration) {
		entry = p.initEntryUnsafe(id, p.defaultDuration)
	} else {
...
func (p *Backoff) NextWithInitDuration(id string, backoffDuration time.Duration, eventTime time.Time) {
	p.Lock()
	defer p.Unlock()
	entry, ok := p.perItemBackoff[id]
	if !ok || hasExpired(eventTime, entry.lastUpdate, p.maxDuration) {
