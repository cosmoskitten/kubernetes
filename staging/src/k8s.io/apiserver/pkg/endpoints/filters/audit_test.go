...
func TestAuditLegacy(t *testing.T) {
	...
	for _, test := range []struct {
		...
		},
		{
			...
			shortRunningPath,
			"GET",
			[]auditinternal.Stage{auditinternal.Stage("RequestReceived")},
			func(http.ResponseWriter, *http.Request) {},
			[]string{
				readOnlyShortRunningPrefix(auditinternal.StageResponseComplete) + ` response="200"`,
			},
		},
		{
			...
			longRunningPath,
			"GET",
			[]auditinternal.Stage{auditinternal.Stage("RequestReceived"), auditinternal.Stage("ResponseStarted"), auditinternal.Stage("ResponseComplete")},
			func(w http.ResponseWriter, req *http.Request) {
				w.Write([]byte("foo"))
				panic("kaboom")
			},
