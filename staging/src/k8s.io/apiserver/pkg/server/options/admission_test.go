...
func TestGetEnabledPluginNamesMethod(t *testing.T) {
	scenarios := []struct {
		expectedPluginNames       []string
		setDefaultOffPluginNames  []string
		setRecommendedPluginOrder []string
	}{
		// scenario 1: check if a call to getEnabledPluginNames sets expected values.
		{
	...
	for index, scenario := range scenarios {
		t.Run(fmt.Sprintf("scenario %d", index), func(t *testing.T) {
			...
			actualPluginNames := target.getEnabledPluginNames()
