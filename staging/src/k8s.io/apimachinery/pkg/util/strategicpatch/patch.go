...
func handleMapDiff(key string, originalValue, modifiedValue, patch map[string]interface{},
	...
	subschema, patchMeta, err := schema.LookupPatchMetadata(key)
	...
	retainKeys, patchStrategy, err := extractRetainKeysPatchStrategy(patchMeta.PatchStrategies)
	if err != nil {
		return err
	}
...
func handleSliceDiff(key string, originalValue, modifiedValue []interface{}, patch map[string]interface{},
	...
	case mergeDirective:
		diffOptions.BuildRetainKeysDirective = retainKeys
		addList, deletionList, setOrderList, err := diffLists(originalValue, modifiedValue, subschema.Elem(), patchMeta.PatchMergeKey, diffOptions)
		if err != nil {
			return err
		}
		if len(addList) > 0 {
...
func mergePatchIntoOriginal(original, patch map[string]interface{}, schema LookupPatchMeta, mergeOptions MergeOptions) error {
	for key, patchV := range patch {
		...
		subschema, patchMeta, err = schema.LookupPatchMetadata(originalKey)
		if err != nil {
			return err
		}
		...
		err = validatePatchWithSetOrderList(patchFieldValue, typedSetElementOrderList, patchMeta.PatchMergeKey)
		if err != nil {
			return err
		}
		...
		case foundOriginal && foundPatch:
			merged, err = mergeSliceHandler(originalList, patchList, subschema,
				patchStrategy, patchMeta.PatchMergeKey, false, mergeOptions)
			if err != nil {
				return err
			}
		case !foundOriginal && !foundPatch:
		...
		if len(patchMeta.PatchMergeKey) == 0 {
			...
			patchItems, serverOnlyItems, err = partitionMapsByPresentInList(merged, typedSetElementOrderList, patchMeta.PatchMergeKey)
			if err != nil {
				return err
			}
		}
		...
		both, err := normalizeElementOrder(patchItems, serverOnlyItems, typedSetElementOrderList, originalFieldValue, patchMeta.PatchMergeKey, kind)
		if err != nil {
			return err
		}
...
func mergeMap(original, patch map[string]interface{}, schema LookupPatchMeta, mergeOptions MergeOptions) (map[string]interface{}, error) {
	...
	for k, patchV := range patch {
		...
		subschema, patchMeta, err := schema.LookupPatchMetadata(k)
		if err != nil {
			return nil, err
		}
		_, patchStrategy, err := extractRetainKeysPatchStrategy(patchMeta.PatchStrategies)
		...
		case reflect.Slice:
			original[k], err = mergeSliceHandler(original[k], patchV, subschema, patchStrategy, patchMeta.PatchMergeKey, isDeleteList, mergeOptions)
		default:
			original[k] = patchV
		}
		if err != nil {
...
func mergeSliceHandler(original, patch interface{}, schema LookupPatchMeta,
	...
	if fieldPatchStrategy == mergeDirective {
		return mergeSlice(typedOriginal, typedPatch, schema.Elem(), fieldPatchMergeKey, mergeOptions, isDeleteList)
	} else {
		return typedPatch, nil
	}
}
		...
		if k == retainKeysDirective {
			...
			if typedV, ok := v.(map[string]interface{}); ok {
				...
				if patchStrategy == mergeDirective {
					var err error
					subschemaElem := subschema.Elem()
					v, err = sortMergeListsByNameArray(typedV, subschemaElem, patchMeta.PatchMergeKey, true)
					if err != nil {
						return nil, err
					}
				}
		...
		if key != directiveMarker && key != retainKeysDirective {
			if rightValue, ok := typedRight[key]; ok {
				...
				if hasConflicts, err := mergingMapFieldsHaveConflicts(leftValue, rightValue,
					subschema, patchStrategy, patchMeta.PatchMergeKey); hasConflicts {
					return true, err
				}
			}
		}
...
func slicesHaveConflicts(
	...
	if fieldPatchStrategy == mergeDirective {
		...
		return mapsOfMapsHaveConflicts(leftMap, rightMap, schema.Elem())
	}
	...
	for i := range typedLeft {
		if hasConflicts, err := mergingMapFieldsHaveConflicts(typedLeft[i], typedRight[i], schema.Elem(), "", ""); hasConflicts {
			return true, err
		}
	}
