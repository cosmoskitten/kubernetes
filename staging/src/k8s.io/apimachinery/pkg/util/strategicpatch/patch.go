...
func diffListsOfMaps(original, modified []interface{}, t reflect.Type, mergeKeys []string, diffOptions DiffOptions) ([]interface{}, error) {
	...
	for {
		...
		case bothInBounds && ItemMatchesOriginalAndModifiedSlice(compareResult):
			...
			if len(patchValue) > 0 {
				addMergeKeysToMap(patchValue, mergeKeys, modifiedElementMergeKeysMap)
		...
		case bothInBounds && ItemRemovedFromModifiedSlice(compareResult):
			if !diffOptions.IgnoreDeletions {
				...
				patch = append(patch, CreateDeleteDirective(mergeKeys, originalElementMergeKeysMap))
			}
			originalIndex++
		}
	}
...
func CreateDeleteDirective(mergeKeys []string, mergeKeysMap map[string]interface{}) map[string]interface{} {
	m := map[string]interface{}{directiveMarker: deleteDirective}
	addMergeKeysToMap(m, mergeKeys, mergeKeysMap)
	return m
}

func addMergeKeysToMap(m map[string]interface{}, mergeKeys []string, mergeKeysMap map[string]interface{}) {
	for _, mergeKey := range mergeKeys {
