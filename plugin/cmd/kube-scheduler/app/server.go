...
func (o *Options) applyDeprecatedHealthzPortToConfig() {
	...
	index := strings.Index(o.config.HealthzBindAddress, ":")
	if index != -1 {
		addr := o.config.HealthzBindAddress[:index]
		bindAddr := fmt.Sprintf("%s:%d", addr, o.healthzPort)
		o.config.HealthzBindAddress = bindAddr
		o.config.MetricsBindAddress = bindAddr
	}
}
...
func (o *Options) applyDeprecatedAlgorithmSourceOptionsToConfig() {
	...
	case o.useLegacyPolicyConfig:
		o.config.AlgorithmSource = componentconfig.SchedulerAlgorithmSource{
			Type: componentconfig.SchedulerAlgorithmSourcePolicy,
			Policy: &componentconfig.SchedulerPolicySource{
				Type: componentconfig.SchedulePolicySourceTypeFile,
	...
	case len(o.policyConfigMapName) > 0:
		o.config.AlgorithmSource = componentconfig.SchedulerAlgorithmSource{
			Type: componentconfig.SchedulerAlgorithmSourcePolicy,
			Policy: &componentconfig.SchedulerPolicySource{
				Type: componentconfig.SchedulerPolicySourceTypeConfigMap,
	...
	case len(o.algorithmProvider) > 0:
		o.config.AlgorithmSource = componentconfig.SchedulerAlgorithmSource{
			Type:     componentconfig.SchedulerAlgorithmSourceProvider,
...
func (o *Options) Run() error {
	...
	server, err := NewSchedulerServer(config, o.scheme, o.master)
	if err != nil {
		return err
	}
...
func NewSchedulerServer(config *componentconfig.KubeSchedulerConfiguration, scheme *runtime.Scheme, master string) (*SchedulerServer, error) {
	if config == nil {
	...
	recorder := eventBroadcaster.NewRecorder(scheme, v1.EventSource{Component: config.SchedulerName})
	...
	if config.LeaderElection.LeaderElect {
		...
		rl, err := resourcelock.New(config.LeaderElection.ResourceLock,
			config.LeaderElection.LockObjectNamespace,
			config.LeaderElection.LockObjectName,
			client.CoreV1(),
...
func (s *SchedulerServer) Run() error {
	...
	switch s.AlgorithmSource.Type {
	case componentconfig.SchedulerAlgorithmSourceProvider:
	...
	case componentconfig.SchedulerAlgorithmSourcePolicy:
		...
		switch s.AlgorithmSource.Policy.Type {
		case componentconfig.SchedulePolicySourceTypeFile:
			// Use a policy serialized in a file.
			policyFile := s.AlgorithmSource.Policy.File.Path
		...
		case componentconfig.SchedulerPolicySourceTypeConfigMap:
			// Use a policy serialized in a config map value.
			policyRef := s.AlgorithmSource.Policy.ConfigMap.Reference
			...
			if !found {
				return fmt.Errorf("missing policy config map value at key %q", componentconfig.SchedulerPolicyConfigMapKey)
			}
			err = runtime.DecodeInto(latestschedulerapi.Codec, []byte(data), policy)
			if err != nil {
				return fmt.Errorf("invalid policy: %v", err)
		...
		if err != nil {
			return fmt.Errorf("couldn't create scheduler from policy: %v", err)
		}
		schedulerConfig = sc
	}

	// Additional tweaks to the config produced by the configurator.
	schedulerConfig.Recorder = s.Recorder
...
func createClients(config componentconfig.ClientConnectionConfiguration, masterOverride string) (clientset.Interface, v1core.EventsGetter, error) {
	...
	client, err := clientset.NewForConfig(restclient.AddUserAgent(kubeConfig, "leader-election"))
	if err != nil {
		return nil, nil, err
	}
