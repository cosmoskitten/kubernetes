...
func makeBasePod(t *testing.T, nodeName, objName, cpu, mem, oir string, ports []v1.ContainerPort) *v1.Pod {
	req := v1.ResourceList{}
	if cpu != "" {
		req = v1.ResourceList{
		...
		if oir != "" {
			if len(strings.Split(oir, ":")) != 2 && t != nil {
				t.Fatalf("Invalid OIR string: %v", oir)
			}
...
func setupCacheOf1kNodes30kPods(b *testing.B) Cache {
	...
	for i := 0; i < 1000; i++ {
		...
		for j := 0; j < 30; j++ {
			...
			pod := makeBasePod(nil, nodeName, objName, "0", "0", "", nil)
