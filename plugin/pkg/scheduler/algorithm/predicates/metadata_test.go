...
func TestPredicateMetadata_AddRemovePod(t *testing.T) {
	...
	for _, test := range tests {
		...
		assert.NoError(t, err, "test [%v]: error adding pod to meta: %v", test.description, err)
		err = predicateMetadataEquivalent(allPodsMeta, existingPodsMeta1)
		assert.NoError(t, err, "test [%v]: meta data are not equivalent: %v", test.description, err)
		...
		assert.NoError(t, err, "test [%v]: error removing pod from meta: %v", test.description, err)
