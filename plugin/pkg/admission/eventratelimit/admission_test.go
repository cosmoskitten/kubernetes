...
func TestEventRateLimiting(t *testing.T) {
	cases := []struct {
		...
		sourceObjectBurst     int
		sourceObjectCacheSize int
		userBurst             int
		userCacheSize         int
		requests              []request
	}{
...
TESTCASESLOOP:
	for _, tc := range cases {
		...
		eventratelimit, err := newEventRateLimitWithClock(config, clock)
		if err != nil {
			t.Errorf("%v: Could not create EventRateLimit: %v", tc.name, err)
		...
		for rqIndex, rq := range tc.requests {
			...
			if rq.accepted != (err == nil) {
				...
				t.Errorf("%v: Request %v should have been %v: %v", tc.name, rqIndex, expectedAction, err)
				continue TESTCASESLOOP
			}
			if err != nil {
				statusErr, ok := err.(*errors.StatusError)
				if ok && statusErr.ErrStatus.Code != errors.StatusTooManyRequests {
					t.Errorf("%v: Request %v should yield a 429 response: %v", tc.name, rqIndex, err)
					continue TESTCASESLOOP
				}
			}
		}
