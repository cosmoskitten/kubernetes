...
func ValidateConfiguration(config *eventratelimitapi.Configuration) field.ErrorList {
	...
	for i, limit := range config.Limits {
		...
		if limit.Burst >= math.MaxInt32 {
			allErrs = append(allErrs, field.Invalid(idxPath.Child("burst"), limit.Burst, fmt.Sprintf("must be less than %v", math.MaxInt32)))
		}
		if limit.QPS <= 0 {
		...
		if limit.Type != eventratelimitapi.ServerLimitType {
			...
			if limit.CacheSize >= math.MaxInt32 {
				allErrs = append(allErrs, field.Invalid(idxPath.Child("cacheSize"), limit.CacheSize, fmt.Sprintf("must be less than %v", math.MaxInt32)))
			}
		}
	}
	return allErrs
