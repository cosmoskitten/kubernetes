...
func mergeAPIResourceConfigs(defaultAPIResourceConfig *serverstorage.ResourceConfig, resourceConfigOverrides utilflag.ConfigurationMap) (*serverstorage.ResourceConfig, error) {
	...
	if ok {
		if allAPIFlagValue == "false" {
			...
			resourceConfig.DisableVersions(globalscheme.Registry.RegisteredGroupVersions()...)
		} else if allAPIFlagValue == "true" {
			resourceConfig.EnableVersions(globalscheme.Registry.RegisteredGroupVersions()...)
		}
	}
	...
	for key := range overrides {
		...
		// Verify that the groupVersion is globalscheme.Registry.
		if !globalscheme.Registry.IsRegisteredVersion(groupVersion) {
