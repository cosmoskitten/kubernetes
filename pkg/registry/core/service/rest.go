...
// so small that we expect a linear search to be faster
func containsNodePorts(serviceNodePorts []ServiceNodePort, serviceNodePort ServiceNodePort) bool {
	for _, snp := range serviceNodePorts {
		if snp.Protocol == serviceNodePort.Protocol && snp.NodePort == serviceNodePort.NodePort {
			return true
		}
	}
	return false
...
func (rs *REST) updateNodePort(oldService, newService *api.Service, nodePortOp *portallocator.PortAllocationOperation) error {
	...
	nodePortAllocated := map[int]bool{}
	...
	for i := range newService.Spec.Ports {
		...
		if nodePort.NodePort != 0 {
			if !containsNumber(oldNodePortsNumbers, int(nodePort.NodePort)) && !nodePortAllocated[int(nodePort.NodePort)] {
				...
				nodePortAllocated[int(nodePort.NodePort)] = true
			}
		} else {
		...
		if containsNodePorts(newNodePorts, nodePort) {
