...
func TestRangeSize(t *testing.T) {
	testCases := []struct {
		...
		{
			name:   "success IPv4 /24 cidr",
			cidr:   "192.168.1.0/24",
			addrs:  256,
			expect: true,
		},
		{
			name:   "failure unsupported IPv4 cidr",
			cidr:   "192.168.1.0/1",
			expect: false,
		},
		{
			name:   "success IPv6 /98 cidr",
			cidr:   "2001:db8::/98",
			addrs:  1073741824,
			expect: true,
		},
		{
			name:   "failure unsupported IPv6 cidr",
			cidr:   "2001:db8::/65",
			expect: false,
		},
		{
			name:   "failure invalid IPv4 mask",
			cidr:   "2001:db8::/65",
			expect: false,
		},
	}
	...
	for _, tc := range testCases {
		...
		size, err := RangeSize(cidr)
		switch {
		case err == nil && tc.addrs != 0:
			if (size != tc.addrs) && tc.expect {
				t.Errorf("test %s failed. %s should have a range size of %d, got %d",
					tc.name, tc.cidr, tc.addrs, size)
			}
			if (size == tc.addrs) && !tc.expect {
				t.Errorf("test %s failed. %s should not have a range size of %d, got %d",
					tc.name, tc.cidr, tc.addrs, size)
			}
		case err != ErrMaskTooSmall:
			t.Errorf("test %s failed. expected error %s, got %s",
				tc.name, ErrMaskTooSmall, err)
		}
	}
}
