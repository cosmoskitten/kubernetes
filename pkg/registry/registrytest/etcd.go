...
func NewEtcdStorage(t *testing.T, group string) (*storagebackend.Config, *etcdtesting.EtcdTestServer) {
	server, config := etcdtesting.NewUnsecuredEtcd3TestClientServer(t, globalscheme.Scheme)
...
func New(t *testing.T, storage *genericregistry.Store) *Tester {
	return &Tester{
		tester:  resttest.New(t, storage, globalscheme.Scheme),
		storage: storage,
	}
}
...
func getCodec(obj runtime.Object) (runtime.Codec, error) {
	fqKinds, _, err := globalscheme.Scheme.ObjectKinds(obj)
	if err != nil {
	...
	if globalscheme.Scheme.Recognizes(globalscheme.Registry.GroupOrDie(api.GroupName).GroupVersion.WithKind(fqKind.Kind)) {
		codec = testapi.Default.Codec()
	} else if globalscheme.Scheme.Recognizes(testapi.Extensions.GroupVersion().WithKind(fqKind.Kind)) {
