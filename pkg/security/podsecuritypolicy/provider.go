...
func (s *simpleProvider) CreatePodSecurityContext(pod *api.Pod) (*api.PodSecurityContext, map[string]string, error) {
	var sc securitycontext.PodSecurityContextMutator
	if pod.Spec.SecurityContext != nil {
		// work with a copy
		copy := *pod.Spec.SecurityContext
		sc = securitycontext.NewPodSecurityContextMutator(&copy)
	} else {
...
func (s *simpleProvider) CreateContainerSecurityContext(pod *api.Pod, container *api.Container) (*api.SecurityContext, map[string]string, error) {
	var podSC securitycontext.PodSecurityContextMutator
	if pod.Spec.SecurityContext != nil {
		// work with a copy of the original
		copy := *pod.Spec.SecurityContext
		podSC = securitycontext.NewPodSecurityContextMutator(&copy)
	} else {
		podSC = securitycontext.NewPodSecurityContextMutator(nil)
	}

	var containerSC securitycontext.ContainerSecurityContextMutator
	if container.SecurityContext != nil {
		// work with a copy of the original
		copy := *container.SecurityContext
		containerSC = securitycontext.NewContainerSecurityContextMutator(&copy)
	} else {
		containerSC = securitycontext.NewContainerSecurityContextMutator(nil)
	}

	sc := securitycontext.NewEffectiveContainerSecurityContextMutator(podSC, containerSC)
...
func (s *simpleProvider) ValidatePodSecurityContext(pod *api.Pod, fldPath *field.Path) field.ErrorList {
	...
	sc := securitycontext.PodSecurityContextAccessor(securitycontext.NewPodSecurityContextMutator(pod.Spec.SecurityContext))
...
func (s *simpleProvider) ValidateContainerSecurityContext(pod *api.Pod, container *api.Container, fldPath *field.Path) field.ErrorList {
	...
	podSC := securitycontext.PodSecurityContextAccessor(securitycontext.NewPodSecurityContextMutator(pod.Spec.SecurityContext))
	sc := securitycontext.ContainerSecurityContextAccessor(securitycontext.NewEffectiveContainerSecurityContextMutator(podSC, securitycontext.NewContainerSecurityContextMutator(container.SecurityContext)))
