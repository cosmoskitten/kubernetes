...
func (m *kubeGenericRuntimeManager) computePodContainerChanges(pod *v1.Pod, podStatus *kubecontainer.PodStatus, existingStatus v1.PodStatus) podContainerSpecChanges {
	...
	for index, container := range pod.Spec.Containers {
		...
		var recordedContainerState v1.ContainerState
		if len(existingStatus.ContainerStatuses) > index {
			recordedContainerState = existingStatus.ContainerStatuses[index].State
		}
		...
		if containerStatus == nil {
			...
			// The container isn't running, but we don't have a Terminated state for it. Record that it exited.
			if recordedContainerState.Running != nil {
