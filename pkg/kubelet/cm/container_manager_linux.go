...
func NewContainerManager(mountUtil mount.Interface, cadvisorInterface cadvisor.Interface, nodeConfig NodeConfig, failSwapOn bool, recorder record.EventRecorder) (ContainerManager, error) {
	...
	// error out unless --fail-on-swap is set to false.
	if len(buf) > 1 {
		if failSwapOn {
			return nil, fmt.Errorf("Running with swap on is not supported, please disable swap! or set --fail-on-swap flag to false. /proc/swaps contained: %v", buf)
		}
	}
