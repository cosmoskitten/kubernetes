...
type DevicePluginManagerStub struct {
...
func NewDevicePluginManagerStub() (*DevicePluginManagerStub, error) {
	return &DevicePluginManagerStub{
...
func (m *DevicePluginManagerStub) Start() error {
	return nil
}

func (m *DevicePluginManagerStub) Devices() map[string][]*pluginapi.Device {
	return make(map[string][]*pluginapi.Device)
}

func (m *DevicePluginManagerStub) Allocate(resourceName string, devIds []string) (*pluginapi.AllocateResponse, error) {
...
func (m *DevicePluginManagerStub) Stop() error {
	return nil
}
...
func TestPodContainerDeviceAllocation(t *testing.T) {
	...
	m, err := NewDevicePluginManagerStub()
	as := assert.New(t)
	as.Nil(err)
	monitorCallback := func(resourceName string, added, updated, deleted []*pluginapi.Device) {}
	updateCapacityCallback := func(v1.ResourceList) {}
	...
	as.Equal(runContainerOpts.Devices[0].PathOnHost, "/dev/aaa")
	as.Equal(runContainerOpts.Devices[1].PathOnHost, "/dev/bbb")
	as.Equal(runContainerOpts.Devices[2].PathOnHost, "/dev/ccc")
	...
	as.Equal(runContainerOpts.Envs[0], kubecontainer.EnvVar{"key1", "val1"})
	as.Equal(runContainerOpts.Envs[1], kubecontainer.EnvVar{"key2", "val2"})
