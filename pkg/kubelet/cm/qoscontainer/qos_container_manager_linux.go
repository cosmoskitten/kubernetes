...
type QOSContainerManager interface {
	Start(func() v1.ResourceList, cmtypes.ActivePodsFunc) error
...
type qosContainerManagerImpl struct {
	sync.Mutex
	nodeInfo           *v1.Node
	qosContainersInfo  cmtypes.QOSContainersInfo
	subsystems         *cmtypes.CgroupSubsystems
	cgroupManager      cmtypes.CgroupManager
	activePods         cmtypes.ActivePodsFunc
	getNodeAllocatable func() v1.ResourceList
	cgroupRoot         string
	qosReserved        map[v1.ResourceName]int64
}

func NewQOSContainerManager(subsystems *cmtypes.CgroupSubsystems, cgroupRoot string, nodeConfig cmtypes.NodeConfig) (QOSContainerManager, error) {
	if !nodeConfig.CgroupsPerQOS {
		return &qosContainerManagerNoop{
			cgroupRoot: cmtypes.CgroupName(nodeConfig.CgroupRoot),
		}, nil
	}
...
func (m *qosContainerManagerImpl) GetQOSContainersInfo() cmtypes.QOSContainersInfo {
	return m.qosContainersInfo
}

func (m *qosContainerManagerImpl) Start(getNodeAllocatable func() v1.ResourceList, activePods cmtypes.ActivePodsFunc) error {
	cm := m.cgroupManager
	rootContainer := m.cgroupRoot
	if !cm.Exists(cmtypes.CgroupName(rootContainer)) {
	...
	for qosClass, containerName := range qosClasses {
		...
		absoluteContainerName := cmtypes.CgroupName(containerName)

		resourceParameters := &cmtypes.ResourceConfig{}
		// the BestEffort QoS class has a statically configured minShares value
		if qosClass == v1.PodQOSBestEffort {
			minShares := uint64(cmhelper.MinShares)
	...
	m.qosContainersInfo = cmtypes.QOSContainersInfo{
...
func (m *qosContainerManagerImpl) setHugePagesUnbounded(cgroupConfig *cmtypes.CgroupConfig) error {
...
func (m *qosContainerManagerImpl) setHugePagesConfig(configs map[v1.PodQOSClass]*cmtypes.CgroupConfig) error {
...
func (m *qosContainerManagerImpl) setCPUCgroupConfig(configs map[v1.PodQOSClass]*cmtypes.CgroupConfig) error {
	pods := m.activePods()
	...
	bestEffortCPUShares := uint64(cmhelper.MinShares)
	configs[v1.PodQOSBestEffort].ResourceParameters.CpuShares = &bestEffortCPUShares

	// set burstable shares based on current observe state
	burstableCPUShares := cmhelper.MilliCPUToShares(burstablePodCPURequest)
	if burstableCPUShares < uint64(cmhelper.MinShares) {
		burstableCPUShares = uint64(cmhelper.MinShares)
	}
...
func (m *qosContainerManagerImpl) setMemoryReserve(configs map[v1.PodQOSClass]*cmtypes.CgroupConfig, percentReserve int64) {
...
func (m *qosContainerManagerImpl) retrySetMemoryReserve(configs map[v1.PodQOSClass]*cmtypes.CgroupConfig, percentReserve int64) {
...
func (m *qosContainerManagerImpl) UpdateCgroups() error {
	...
	qosConfigs := map[v1.PodQOSClass]*cmtypes.CgroupConfig{
		v1.PodQOSBurstable: {
			Name:               cmtypes.CgroupName(m.qosContainersInfo.Burstable),
			ResourceParameters: &cmtypes.ResourceConfig{},
		},
		v1.PodQOSBestEffort: {
			Name:               cmtypes.CgroupName(m.qosContainersInfo.BestEffort),
			ResourceParameters: &cmtypes.ResourceConfig{},
		},
	}
...
type qosContainerManagerNoop struct {
	cgroupRoot cmtypes.CgroupName
}

var _ QOSContainerManager = &qosContainerManagerNoop{}

func (m *qosContainerManagerNoop) GetQOSContainersInfo() cmtypes.QOSContainersInfo {
	return cmtypes.QOSContainersInfo{}
}

func (m *qosContainerManagerNoop) Start(_ func() v1.ResourceList, _ cmtypes.ActivePodsFunc) error {
	return nil
}
