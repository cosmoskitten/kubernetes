...
func (cm *containerManagerImpl) createNodeAllocatableCgroups() error {
	cgroupConfig := &cmtypes.CgroupConfig{
		Name: cmtypes.CgroupName(cm.cgroupRoot),
...
func enforceExistingCgroup(cgroupManager cmtypes.CgroupManager, cName string, rl v1.ResourceList) error {
	cgroupConfig := &CgroupConfig{
...
func getCgroupConfig(rl v1.ResourceList) *cmtypes.ResourceConfig {
	...
	if q, exists := rl[v1.ResourceCPU]; exists {
		...
		val := cmhelper.MilliCPUToShares(q.MilliValue())
		rc.CpuShares = &val
	}
