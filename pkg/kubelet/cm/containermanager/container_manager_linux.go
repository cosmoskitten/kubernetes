...
import (
	...
	containerlifecycle "k8s.io/kubernetes/pkg/kubelet/cm/containerlifecycle"
...
func NewContainerManager(mountUtil mount.Interface, cadvisorInterface cadvisor.Interface, nodeConfig cmtypes.NodeConfig, failSwapOn bool, devicePluginEnabled bool, recorder record.EventRecorder) (ContainerManager, error) {
	subsystems, err := cmhelper.GetCgroupSubsystems()
	if err != nil {
	...
	cgroupManager := cgroupmanager.NewCgroupManager(subsystems, cmtypes.nodeConfig.CgroupDriver)
	...
	if nodeConfig.CgroupsPerQOS {
		...
		if !cgroupManager.Exists(cmtypes.CgroupName(cgroupRoot)) {
...
func (cm *containerManagerImpl) NewPodContainerManager() cmtypes.PodContainerManager {
...
func (cm *containerManagerImpl) setupNode(activePods cmtypes.ActivePodsFunc) error {
...
func (cm *containerManagerImpl) GetNodeConfig() cmtypes.NodeConfig {
	cm.RLock()
	defer cm.RUnlock()
	return cm.NodeConfig
}

func (cm *containerManagerImpl) GetMountedSubsystems() *cmtypes.CgroupSubsystems {
	return cm.subsystems
}

func (cm *containerManagerImpl) GetQOSContainersInfo() cmtypes.QOSContainersInfo {
