...
func (m *podContainerManagerImpl) EnsureExists(pod *v1.Pod) error {
	...
	if !alreadyExists {
		...
		containerConfig := &cmtypes.CgroupConfig{
			...
			ResourceParameters: cmhelper.ResourceConfigForPod(pod),
		}
...
func (m *podContainerManagerImpl) GetPodContainerName(pod *v1.Pod) (cmtypes.CgroupName, string) {
	...
	cgroupName := (cmtypes.CgroupName)(path.Join(parentContainer, podContainer))
...
func (m *podContainerManagerImpl) tryKillingCgroupProcesses(podCgroup cmtypes.CgroupName) error {
...
func (m *podContainerManagerImpl) Destroy(podCgroup cmtypes.CgroupName) error {
...
func (m *podContainerManagerImpl) ReduceCPULimits(podCgroup cmtypes.CgroupName) error {
...
func (m *podContainerManagerImpl) GetAllPodsFromCgroups() (map[types.UID]cmtypes.CgroupName, error) {
	// Map for storing all the found pods on the disk
	foundPods := make(map[types.UID]cmtypes.CgroupName)
	...
	for _, val := range m.subsystems.MountPoints {
		for _, qosContainerName := range qosContainersList {
			...
			qcConversion := m.cgroupManager.Name(cmtypes.CgroupName(qosContainerName))
...
func (m *podContainerManagerNoop) GetPodContainerName(_ *v1.Pod) (cmtypes.CgroupName, string) {
...
// Destroy destroys the pod container cgroup paths
func (m *podContainerManagerNoop) Destroy(_ cmtypes.CgroupName) error {
	return nil
}

func (m *podContainerManagerNoop) ReduceCPULimits(_ cmtypes.CgroupName) error {
	return nil
}

func (m *podContainerManagerNoop) GetAllPodsFromCgroups() (map[types.UID]cmtypes.CgroupName, error) {
	return nil, nil
}
