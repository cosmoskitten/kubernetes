...
func (ngm *nvidiaGPUManager) Start() error {
	...
	go func() {
		for {
			if found := ngm.discoverGPUs(devicesProcFile, devDirectory); found == nil {
				return
			}
...
func (ngm *nvidiaGPUManager) discoverGPUs(devicesFile string, devDir string) error {
	...
	if !expected {
		return errors.New("no GPU cards detected in" + devicesFile)
	}
	...
	if foundGPUs.Len() == 0 {
		return errors.New("expected GPUs, but did not find any under" + devDir)
	}
