...
func ValidateKubeletConfiguration(kc *kubeletconfig.KubeletConfiguration) error {
	...
	if kc.CAdvisorPort < 0 || kc.CAdvisorPort > 65535 {
	...
	if kc.Port < 0 || kc.Port > 65535 {
		allErrors = append(allErrors, fmt.Errorf("Invalid configuration: Port (--port) %v must be between 0 and 65535, inclusive", kc.Port))
	}
	if kc.ReadOnlyPort < 0 || kc.ReadOnlyPort > 65535 {
