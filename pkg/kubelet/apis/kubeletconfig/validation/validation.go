...
func ValidateKubeletConfiguration(kc *kubeletconfig.KubeletConfiguration) error {
	...
	if utilvalidation.IsValidPortNum(int(kc.CAdvisorPort)) != nil {
		allErrors = append(allErrors, fmt.Errorf("Invalid configuration: CAdvisorPort (--cadvisor-port) %v must be between 1 and 65535, inclusive", kc.CAdvisorPort))
	}
	if kc.EventBurst < 0 {
	...
	if utilvalidation.IsValidPortNum(int(kc.ReadOnlyPort)) != nil {
		allErrors = append(allErrors, fmt.Errorf("Invalid configuration: ReadOnlyPort (--read-only-port) %v must be between 1 and 65535, inclusive", kc.ReadOnlyPort))
	}
