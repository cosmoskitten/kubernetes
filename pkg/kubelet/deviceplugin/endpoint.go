...
func (e *endpoint) ListAWatch(stream pluginapi.DevicePlugin_ListAndWatchClient) {
	glog.V(2).Infof("Starting ListAndWatch")
	...
	for {
		...
		for _, d := range devs {
			...
			if !ok {
				glog.Errorf("New device for Endpoint %s: %v", e.resourceName, d)
				continue
			}
