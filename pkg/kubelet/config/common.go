...
func getSelfLink(name, namespace string) string {
	...
	selfLink = fmt.Sprintf("/api/"+globalscheme.Registry.GroupOrDie(api.GroupName).GroupVersion.Version+"/namespaces/%s/pods/%s", namespace, name)
	return selfLink
}
...
func tryDecodeSinglePod(data []byte, defaultFn defaultFunc) (parsed bool, pod *v1.Pod, err error) {
	...
	obj, err := runtime.Decode(globalscheme.Codecs.UniversalDecoder(), json)
	if err != nil {
...
func tryDecodePodList(data []byte, defaultFn defaultFunc) (parsed bool, pods v1.PodList, err error) {
	obj, err := runtime.Decode(globalscheme.Codecs.UniversalDecoder(), data)
	if err != nil {
