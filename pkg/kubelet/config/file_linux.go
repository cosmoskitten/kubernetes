...
func (s *sourceFileListerWatcher) doWatch() error {
	...
	for {
		select {
		case event := <-w.Event:
			err = s.produceWatchEvent(event)
			if err != nil {
				glog.Errorf("error while processing inotify event (%+v): %v", event, err)
			}
		case err = <-w.Error:
...
func (s *sourceFileListerWatcher) produceWatchEvent(e *inotify.Event) error {
...
func (s *sourceFileListerWatcher) consumeWatchEvent(e *watchEvent) error {
	...
	case podAdd, podModify:
		if pod, err := s.extractFromFile(e.fileName); err != nil {
			glog.Errorf("can't process config file %q: %v", e.fileName, err)
		} else {
	...
	case podDelete:
		if objKey, keyExist := s.fileKeyMapping[e.fileName]; keyExist {
			...
			if err != nil {
				...
				glog.Errorf("the pod with key %s doesn't exist in cache", objKey)
				return nil
			} else {
