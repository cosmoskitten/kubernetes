...
func NewJSONEncoder(groupName string) (runtime.Encoder, error) {
	...
	versions := globalscheme.Registry.EnabledVersionsForGroup(groupName)
	if len(versions) == 0 {
		return nil, fmt.Errorf("no enabled versions for group %q", groupName)
	}

	// the "best" version supposedly comes first in the list returned from globalscheme.Registry.EnabledVersionsForGroup
	return globalscheme.Codecs.EncoderForVersion(info.Serializer, versions[0]), nil
}
