...
func TestCPUAccumulatorTake(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		for _, cpus := range tc.takeCPUs {
			if cpus.Intersection(acc.availableCPUs).Size() > 0 {
				t.Errorf("[%s] expected intersection of taken cpus [%s] and acc.availableCPUs [%s] to be empty", tc.description, cpus, tc.availableCPUs)
			}
