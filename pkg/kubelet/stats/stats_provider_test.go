	...
	stats := cadvisorapiv2.ContainerStats{
		...
		Network: &cadvisorapiv2.NetworkStats{
			Interfaces: []cadvisorapiv1.InterfaceStats{{
				...
				Name:     "cbr0",
				RxBytes:  cbr0NetworkSeed,
				RxErrors: cbr0NetworkSeed,
				TxBytes:  cbr0NetworkSeed,
				TxErrors: cbr0NetworkSeed,
			}},
		},
...
func checkNetworkStats(t *testing.T, label string, seed int, stats *statsapi.NetworkStats) {
	...
	assert.EqualValues(t, seed+offsetNetRxBytes+cbr0NetworkSeed, *stats.RxBytes, label+".Net.RxBytes")
	assert.EqualValues(t, seed+offsetNetRxErrors+cbr0NetworkSeed, *stats.RxErrors, label+".Net.RxErrors")
	assert.EqualValues(t, seed+offsetNetTxBytes+cbr0NetworkSeed, *stats.TxBytes, label+".Net.TxBytes")
	assert.EqualValues(t, seed+offsetNetTxErrors+cbr0NetworkSeed, *stats.TxErrors, label+".Net.TxErrors")
}
