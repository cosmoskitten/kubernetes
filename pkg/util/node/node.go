...
func UpdateNodeStatus(c clientset.Interface, nodeName types.NodeName, oldNode *v1.Node, newNode *v1.Node) (*v1.Node, error) {
	...
	updatedNode, err := c.CoreV1().Nodes().PatchStatus(string(nodeName), patchBytes)
	if err != nil {
		return nil, fmt.Errorf("failed to update status %q for node %q: %v", patchBytes, nodeName, err)
	}
	return updatedNode, nil
}
