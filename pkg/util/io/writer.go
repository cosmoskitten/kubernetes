...
func (writer *NsenterWriter) WriteFile(filename string, data []byte, perm os.FileMode) error {
	...
	glog.V(5).Infof("Command to write data to file: nsenter --mount=/rootfs/proc/1/ns/mnt -- sh %v", echoArgs)
	...
	glog.V(5).Infof("Command to change permissions to file: nsenter --mount=/rootfs/proc/1/ns/mnt -- chmod %v", chmodArgs)
