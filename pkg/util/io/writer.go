...
func (writer *NsenterWriter) WriteFile(filename string, data []byte, perm os.FileMode) error {
	...
	glog.V(5).Infof("Command to write data to file: %v %v", nsenterPath, echoArgs)
	command := exec.Command(nsenterPath, echoArgs...)
	...
	glog.V(5).Infof("Command to change permissions to file: %v %v", nsenterPath, chmodArgs)
	outputBytes, err = exec.Command(nsenterPath, chmodArgs...).CombinedOutput()
	if err != nil {
