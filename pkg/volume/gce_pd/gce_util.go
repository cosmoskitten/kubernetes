...
func (gceutil *GCEDiskUtil) CreateVolume(c *gcePersistentDiskProvisioner) (string, int, map[string]string, string, error) {
	...
	} else if zonePresent && zonesPresent && !replicaZonesPresent {
		// 110
		return "", 0, nil, "", fmt.Errorf("both zone and zones StorageClass parameters must not be used at the same time")
	} else if zonePresent && !zonesPresent && replicaZonesPresent {
		// 101
		return "", 0, nil, "", fmt.Errorf("both zone and replica-zones StorageClass parameters must not be used at the same time")
	} else if !zonePresent && zonesPresent && replicaZonesPresent {
		// 011
		return "", 0, nil, "", fmt.Errorf("both zones and replica-zones StorageClass parameters must not be used at the same time")
	}
...
func createRegionalPD(
	...
	glog.V(2).Infof("Successfully created Regional GCE PD volume %s", diskName)
	return nil
}
