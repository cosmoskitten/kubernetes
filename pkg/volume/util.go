...
func internalRecycleVolumeByWatchingPodUntilCompletion(pvName string, pod *v1.Pod, recyclerClient recyclerClient) error {
	...
	if err != nil {
		if errors.IsAlreadyExists(err) {
			...
			if deleteErr != nil {
				glog.V(5).Infof("failed to delete old recycler %s/%s: %s", pod.Namespace, pod.Name, deleteErr)
				return fmt.Errorf("failed to delete old recycler pod: %s", deleteErr)
			}
			glog.V(5).Infof("old recycler pod %q found for volume, deleted", pod.Name)
