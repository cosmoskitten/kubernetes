...
func (plugin *iscsiPlugin) NewMounter(spec *volume.Spec, pod *v1.Pod, _ volume.VolumeOptions) (volume.Mounter, error) {
	...
	chapDiscover, chapSession, err := getISCSICHAPInfo(spec)
	if err != nil {
		return nil, err
	}
	if chapDiscover || chapSession {
		secretNamespace, secretName, err := getISCSISecretNamespaceAndName(spec, pod.Namespace)
		if err != nil {
			return nil, err
		}
...
func getISCSISecretNamespaceAndName(spec *volume.Spec, defaultSecretNamespace string) (string, string, error) {
	if spec.Volume != nil && spec.Volume.ISCSI != nil {
		secretName := ""
