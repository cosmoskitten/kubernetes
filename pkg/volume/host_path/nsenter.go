...
// 4.  The host image must have "file", "touch", "mkdir" binaries in "/bin", "/usr/sbin", or "/usr/bin";
...
func (ftc *nsenterFileTypeChecker) Exists() bool {
	...
	exec := exec.New()
	_, err := exec.Command(nsenterPath, args...).CombinedOutput()
	if err == nil {
		ftc.exists = true
	}
	return ftc.exists
...
func (ftc *nsenterFileTypeChecker) MakeFile() error {
	...
	exec := exec.New()
	if _, err := exec.Command(nsenterPath, args...).CombinedOutput(); err != nil {
		return err
	}
	return nil
}
...
func (ftc *nsenterFileTypeChecker) IsDir() bool {
	return ftc.checkMimetype("inode/directory")
}
...
func (ftc *nsenterFileTypeChecker) IsBlock() bool {
	return ftc.checkMimetype("inode/blockdevice")
}

func (ftc *nsenterFileTypeChecker) IsChar() bool {
	return ftc.checkMimetype("inode/chardevice")
}

func (ftc *nsenterFileTypeChecker) IsSocket() bool {
	return ftc.checkMimetype("inode/socket")
}
