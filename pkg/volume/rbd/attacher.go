...
func (attacher *rbdAttacher) MountDevice(spec *volume.Spec, devicePath string, deviceMountPath string) error {
	...
	err = attacher.plugin.mounter.FormatAndMount(devicePath, deviceMountPath, volumeSource.FSType, mountOptions)
	if err != nil {
		return fmt.Errorf("rbd: failed to mount device %s at %s (fstype: %s), error %v", devicePath, deviceMountPath, volumeSource.FSType, err)
	}
	glog.V(3).Infof("rbd: successfully mount device %s at %s (fstype: %s)", devicePath, deviceMountPath, volumeSource.FSType)
	return nil
}
