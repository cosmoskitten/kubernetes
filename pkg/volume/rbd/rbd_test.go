...
func (fake *fakeDiskManager) DetachDisk(r *rbdPlugin, device string) error {
	fake.mutex.Lock()
	defer fake.mutex.Unlock()
...
func doTestPlugin(t *testing.T, c *testcase) {
	...
	devicePath, err := attacher.WaitForAttach(c.spec, deviceAttachPath, nil, time.Second*10)
	if err != nil {
		t.Fatal(err)
	}
	...
	// mounter
	mounter, err := plug.(*rbdPlugin).newMounterInternal(c.spec, c.podUID, fdm, "secrets")
	if err != nil {
	...
	// unmounter
	unmounter, err := plug.(*rbdPlugin).newUnmounterInternal(c.spec.Name(), c.podUID, fdm)
	if err != nil {
	...
	if err != nil {
		t.Fatalf("Detacher.UnmountDevice failed to unmount", deviceMountPath)
	}
	checkMounterLog(t, fakeMounter, 4, mount.FakeAction{Action: "unmount", Target: c.expectedDeviceMountPath, Source: "", FSType: ""})
	err = detacher.Detach(c.spec, deviceMountPath, fakeNodeName)
	if err != nil {
...
func TestPlugin(t *testing.T) {
	...
	cases = append(cases, &testcase{
		...
		expectedPodMountPath:    fmt.Sprintf("%s/pods/poduid1/volumes/kubernetes.io~rbd/vol1", tmpDir),
	})
	cases = append(cases, &testcase{
		...
		root:                    tmpDir,
		podUID:                  types.UID("poduid2"),
		expectedDevicePath:      "/dev/rbd1",
		expectedDeviceMountPath: fmt.Sprintf("%s/plugins/kubernetes.io/rbd/rbd/pool2-image-image2", tmpDir),
		expectedPodMountPath:    fmt.Sprintf("%s/pods/poduid2/volumes/kubernetes.io~rbd/vol2", tmpDir),
	})
