...
func (og *operationGenerator) GenerateMapVolumeFunc(
	...
	return func() error {
		...
		// Retreive devicePath for none attachable plugin case
		if len(devicePath) == 0 {
			devicePath, err = blockVolumeMapper.GetDevicePath()
			if err != nil {
				// On failure, return error. Caller will log and retry.
				return volumeToMount.GenerateErrorDetailed("MapVolume.GetDevicePath failed", err)
			}
		}
		...
		volumeMapPath := blockVolumeMapper.GetPodDeviceMapPath()
		mapErr = util.MapDevice(devicePath, volumeMapPath, util.PodVolumeDirSymlinkName)
		if mapErr != nil {
			// On failure, return error. Caller will log and retry.
			eventErr, detailedErr := volumeToMount.GenerateError("MapVolume.MapDevice failed", mapErr)
			og.recorder.Eventf(volumeToMount.Pod, v1.EventTypeWarning, kevents.FailedMapVolume, eventErr.Error())
...
func (og *operationGenerator) GenerateUnmapVolumeFunc(
	...
	return func() error {
		...
		podDeviceUnmapPath := blockVolumeUnmapper.GetPodDeviceUnmapPath()
		unmapDeviceErr = util.UnmapDevice(podDeviceUnmapPath, util.PodVolumeDirSymlinkName)
		if unmapDeviceErr != nil {
			// On failure, return error. Caller will log and retry.
			return volumeToUnmount.GenerateErrorDetailed("UnmapVolume.UnmapDevice on pod device map path failed", unmapDeviceErr)
		}
		removeMapPathErr := util.RemoveMapPath(podDeviceUnmapPath)
		if removeMapPathErr != nil {
			// On failure, return error. Caller will log and retry.
			return volumeToUnmount.GenerateErrorDetailed("UnmapVolume.RemoveMapPath failed", removeMapPathErr)
		}
