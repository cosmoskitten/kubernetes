...
func TestFSGroupMount(t *testing.T) {
	...
	errMsg := fmt.Sprintf("cannot mount %s as it has already been mounted with fsGroup %d, but the pod is requesting fsGroup %d", tmpDir, fsGroup1, fsGroup2)
	...
	if err != nil {
		...
		t.Errorf("This test should not return err == nil, fsGroup check logic is broken.")
	}
}
