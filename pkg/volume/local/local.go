...
func (m *localVolumeMounter) SetUpAt(dir string, fsGroup *int64) error {
	...
	if fsGroup != nil {
		...
		if len(refs) > 0 {
			...
			if !fsGroupSame {
				m.plugin.recorder.Eventf(m.pod, v1.EventTypeWarning, "Already Mounted", "%s volume has already been mounted with fsGroup %d, the pod is requesting fsGroup %d, depnding on permissions, infrormation on the volume might not be accessible", m.globalPath, fsGroupOld, fsGroupNew)
			}
		}
