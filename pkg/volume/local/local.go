...
func (m *localVolumeMounter) SetUpAt(dir string, fsGroup *int64) error {
	...
	if fsGroup != nil {
		fsGroupSame, fsGroupOld, err := isSamefsGroup(m.globalPath, fsGroup)
		if err != nil {
			err = fmt.Errorf("failed to check Gid for %s (%v)", m.globalPath, err)
			return err
		}
		if !fsGroupSame {
			err = fmt.Errorf("cannot mount %s as it has already been mounted with Gid %d, but the pod is requesting Gid %d", m.globalPath, fsGroupOld, fsGroup)
			return err
		}
	}
...
func isSamefsGroup(dir string, fsGroup *int64) (bool, int, error) {
	...
	if fsGroup != nil {
		glog.Errorf("fsGroup cannot be nil pointer")
		return false, 0, err
	}
	gidNew := int(*fsGroup)
