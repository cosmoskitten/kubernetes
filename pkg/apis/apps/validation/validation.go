...
func ValidatePersistentVolumeClaimForStatefulset(pvc *api.PersistentVolumeClaim, fldPath *field.Path) field.ErrorList {
	...
	for _, msg := range apivalidation.ValidatePersistentVolumeName(pvc.ObjectMeta.GetName(), false) {
		allErrs = append(allErrs, field.Invalid(fldPath.Child("metadata", "name"), pvc.ObjectMeta.GetName(), msg))
	}
