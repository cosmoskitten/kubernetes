...
func TestV1beta2StatefulSetSpecConversion(t *testing.T) {
	...
	v1beta2Template := v1.PodTemplateSpec{ObjectMeta: metav1.ObjectMeta{Name: "foo"}, Spec: v1.PodSpec{RestartPolicy: v1.RestartPolicy("bar")}}
	apiTemplate := api.PodTemplateSpec{ObjectMeta: metav1.ObjectMeta{Name: "foo"}, Spec: api.PodSpec{RestartPolicy: api.RestartPolicy("bar")}}
	testcases := map[string]struct {
		...
		"StatefulSetSpec Conversion 1": {
			stsSpec1:   &apps.StatefulSetSpec{Replicas: *replicas, Template: apiTemplate},
			stsSepc2:   &v1beta2.StatefulSetSpec{Replicas: replicas, Template: v1beta2Template},
			adjustment: true,
		},
		"StatefulSetSpec Conversion 2": {
			stsSpec1:   &apps.StatefulSetSpec{Replicas: *replicas, Selector: selector, Template: apiTemplate, ServiceName: "foo", PodManagementPolicy: apps.PodManagementPolicyType("bar")},
			stsSepc2:   &v1beta2.StatefulSetSpec{Replicas: replicas, Selector: selector, Template: v1beta2Template, ServiceName: "foo", PodManagementPolicy: v1beta2.PodManagementPolicyType("bar")},
			adjustment: true,
		},
	}
	...
	for k, tc := range testcases {
		...
		if !apiequality.Semantic.DeepDerivative(internal2, tc.stsSpec1) {
...
func TestV1beta2StatefulSetUpdateStrategyConversion(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		"StatefulSetUpdateStrategy Conversion 2": {
			stsUpdateStrategy1: &apps.StatefulSetUpdateStrategy{Type: apps.StatefulSetUpdateStrategyType("foo"), RollingUpdate: appsrollingUpdate},
			stsUpdateStrategy2: &v1beta2.StatefulSetUpdateStrategy{Type: v1beta2.StatefulSetUpdateStrategyType("foo"), RollingUpdate: v1beta2rollingUpdate},
		},
	}
	...
	for k, tc := range testcases {
		...
		if !apiequality.Semantic.DeepDerivative(internal1, tc.stsUpdateStrategy2) {
		...
		if !apiequality.Semantic.DeepDerivative(internal2, tc.stsUpdateStrategy1) {
...
func TestV1beta2RollingUpdateDaemonSetConversion(t *testing.T) {
	...
	for k, tc := range testcases {
		...
		if tc.adjustment {
			if tc.rollingUpdateDs2.MaxUnavailable == nil {
				tc.rollingUpdateDs2.MaxUnavailable = &intstr.IntOrString{}
			}
		}
		if !apiequality.Semantic.DeepDerivative(internal1, tc.rollingUpdateDs2) {
		...
		if !apiequality.Semantic.DeepDerivative(internal2, tc.rollingUpdateDs1) {
...
func TestV1beta2StatefulSetStatusConversion(t *testing.T) {
	...
	*collisionCount = 1
	...
	for k, tc := range testcases {
		...
		if !apiequality.Semantic.DeepDerivative(internal1, tc.stsStatus2) {
		...
		if tc.adjustment {
			if tc.stsStatus1.ObservedGeneration == nil {
				tc.stsStatus1.ObservedGeneration = new(int64)
			}
		}
		if err := api.Scheme.Convert(tc.stsStatus2, internal2, nil); err != nil {
			t.Errorf("%q - %q: unexpected error: %v", k, "from v1beta2 to apps", err)
		}
		if !apiequality.Semantic.DeepDerivative(internal2, tc.stsStatus1) {
...
func TestV1beta2DeploymentConversion(t *testing.T) {
	...
	for k, tc := range testcases {
		...
		if !apiequality.Semantic.DeepDerivative(internal1, tc.deployment2) {
		...
		if tc.adjustment {
			if tc.deployment1.Spec.Template.Spec.SecurityContext == nil {
				tc.deployment1.Spec.Template.Spec.SecurityContext = new(api.PodSecurityContext)
			}
		}
		if !apiequality.Semantic.DeepDerivative(internal2, tc.deployment1) {
...
func TestV1beta2ScaleStatusConversion(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		"ScaleStatus Conversion 3": {
			...
			scaleStatus2: &v1beta2.ScaleStatus{Replicas: 2, Selector: matchLabels, TargetSelector: labelsSelector2.String()},
		},
	}
	...
	for k, tc := range testcases {
		...
		if !apiequality.Semantic.DeepDerivative(internal1, tc.scaleStatus2) {
		...
		if !apiequality.Semantic.DeepDerivative(internal2, tc.scaleStatus1) {
...
func TestV1beta2DeploymentSpecConversion(t *testing.T) {
	...
	extensionsRollbackTo := new(extensions.RollbackConfig)
	extensionsRollbackTo.Revision = int64(2)
	...
	testcases := map[string]struct {
		...
		"DeploymentSpec Conversion 1": {
			deploymentSpec1: &extensions.DeploymentSpec{Replicas: *replica},
			deploymentSpec2: &v1beta2.DeploymentSpec{Replicas: replica},
			adjustment:      true,
		},
		"DeploymentSpec Conversion 2": {
			deploymentSpec1: &extensions.DeploymentSpec{Replicas: *replica, RevisionHistoryLimit: revisionHistoryLimit, MinReadySeconds: 2, Paused: true},
			deploymentSpec2: &v1beta2.DeploymentSpec{Replicas: replica, RevisionHistoryLimit: revisionHistoryLimit, MinReadySeconds: 2, Paused: true},
			adjustment:      true,
		},
		"DeploymentSpec Conversion 3": {
			deploymentSpec1: &extensions.DeploymentSpec{Replicas: *replica, RollbackTo: extensionsRollbackTo},
			deploymentSpec2: &v1beta2.DeploymentSpec{Replicas: replica},
			adjustment:      true,
		},
		"DeploymentSpec Conversion 4": {
			deploymentSpec1: &extensions.DeploymentSpec{Replicas: *replica, ProgressDeadlineSeconds: progressDeadlineSeconds},
			deploymentSpec2: &v1beta2.DeploymentSpec{Replicas: replica, ProgressDeadlineSeconds: progressDeadlineSeconds},
			adjustment:      true,
		},
	}
	...
	for k, tc := range testcases {
		...
		if !apiequality.Semantic.DeepDerivative(internal, tc.deploymentSpec2) {
	...
	for k, tc := range testcases {
		...
		if tc.adjustment {
			if tc.deploymentSpec1.Template.Spec.SecurityContext == nil {
				tc.deploymentSpec1.Template.Spec.SecurityContext = new(api.PodSecurityContext)
			}
		}
		if !apiequality.Semantic.DeepDerivative(internal, tc.deploymentSpec1) {
...
func TestV1beta2DeploymentStrategyConversion(t *testing.T) {
	...
	for k, tc := range testcases {
		...
		if !apiequality.Semantic.DeepDerivative(internal1, tc.deploymentStrategy2) {
		...
		if !apiequality.Semantic.DeepDerivative(internal2, tc.deploymentStrategy1) {
...
func TestV1beta2RollingUpdateDeploymentConversion(t *testing.T) {
	...
	for k, tc := range testcases {
		...
		if !apiequality.Semantic.DeepDerivative(internal1, tc.rollingUpdateDeployment2) {
		...
		if !apiequality.Semantic.DeepDerivative(internal2, tc.rollingUpdateDeployment1) {
	...
	*replicas = 2
	...
	testcases := map[string]struct {
		...
		"ReplicaSetSpec Conversion 1": {
			replicaset1: &extensions.ReplicaSetSpec{Replicas: *replicas, MinReadySeconds: 2},
			replicaset2: &v1beta2.ReplicaSetSpec{Replicas: replicas, MinReadySeconds: 2},
			adjustment:  true,
		},
		"ReplicaSetSpec Conversion 2": {
			replicaset1: &extensions.ReplicaSetSpec{Replicas: *replicas, Selector: selector},
			replicaset2: &v1beta2.ReplicaSetSpec{Replicas: replicas, Selector: selector},
			adjustment:  true,
		},
	}
	...
	for k, tc := range testcases {
		...
		if !apiequality.Semantic.DeepDerivative(internal1, tc.replicaset2) {
		...
		if tc.adjustment {
			if tc.replicaset1.Template.Spec.SecurityContext == nil {
				tc.replicaset1.Template.Spec.SecurityContext = new(api.PodSecurityContext)
			}
		}
		if !apiequality.Semantic.DeepDerivative(internal2, tc.replicaset1) {
