...
func ValidateKubeProxyConfiguration(config *componentconfig.KubeProxyConfiguration) field.ErrorList {
	...
	if _, _, err := net.SplitHostPort(config.BindAddress); err != nil {
		allErrs = append(allErrs, field.Invalid(newPath.Child("BindAddress"), config.BindAddress, err.Error()))
	}

	if _, _, err := net.SplitHostPort(config.HealthzBindAddress); err != nil {
		allErrs = append(allErrs, field.Invalid(newPath.Child("HealthzBindAddress"), config.HealthzBindAddress, err.Error()))
	}
