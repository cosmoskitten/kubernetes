			...
			if yyr2 || yy2arr2 {
				...
				if x.Timezone == nil {
					...
					if false {
						...
						r.EncodeString(codecSelferC_UTF81234, string(yy35))
					}
				}
			} else {
				z.EncSendContainerState(codecSelfer_containerMapKey1234)
				...
				if x.Timezone == nil {
					...
					if false {
						...
						r.EncodeString(codecSelferC_UTF81234, string(yy37))
					}
				}
			}
			if yyr2 || yy2arr2 {
	...
	for yyj3 := 0; ; yyj3++ {
		...
		case "timezone":
			if r.TryDecodeAsNil() {
				...
				if x.Timezone == nil {
					x.Timezone = new(string)
				}
				yym17 := z.DecBinary()
				_ = yym17
				if false {
				} else {
					*((*string)(x.Timezone)) = r.DecodeString()
				}
			}
		default:
			z.DecStructFieldNotFound(-1, yys3)
...
func (x codecSelfer1234) decSliceCronJob(v *[]CronJob, d *codec1978.Decoder) {
	...
	if yyl1 == 0 {
		...
		if yyl1 > cap(yyv1) {
			...
			yyrg1 := len(yyv1) > 0
			yyv21 := yyv1
			yyrl1, yyrt1 = z.DecInferLen(yyl1, z.DecBasicHandle().MaxInitLen, 1200)
			if yyrt1 {
				if yyrl1 <= cap(yyv1) {
					yyv1 = yyv1[:yyrl1]
				} else {
