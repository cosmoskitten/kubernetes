...
func TestSetDefaultJob(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"WQ: Parallelism explicitly 0 and completions unset -> backoffSeconds and BackoffLimit are defaulted": {
			original: &batchv1.Job{
				Spec: batchv1.JobSpec{
		...
		"WQ: Parallelism explicitly 2 and completions unset -> backoffSeconds BackoffLimit are defaulted": {
			original: &batchv1.Job{
				Spec: batchv1.JobSpec{
		...
		"Completions explicitly 2 and others unset -> parallelism, backoffSeconds and BackoffLimit are defaulted": {
			original: &batchv1.Job{
				Spec: batchv1.JobSpec{
		...
		"BackoffLimit explicitly 5 and others unset -> parallelism completions and BackoffSeconds are defaulted": {
			original: &batchv1.Job{
				Spec: batchv1.JobSpec{
		...
		"BackoffSeconds explicitly 5 and others unset -> parallelism completions and BackoffLimit are defaulted": {
			original: &batchv1.Job{
				Spec: batchv1.JobSpec{
					BackoffSeconds: newInt64(5),
					Template: v1.PodTemplateSpec{
						ObjectMeta: metav1.ObjectMeta{Labels: defaultLabels},
					},
				},
	...
	for name, test := range tests {
		...
		validateDefaultInt64(t, name, "BackoffSeconds", actual.Spec.BackoffSeconds, expected.Spec.BackoffSeconds)
...
func validateDefaultInt64(t *testing.T, name string, field string, actual *int64, expected *int64) {
	if (actual == nil) != (expected == nil) {
		t.Errorf("%s: got different *%s than expected: %v %v", name, field, actual, expected)
	}
	if actual != nil && expected != nil {
