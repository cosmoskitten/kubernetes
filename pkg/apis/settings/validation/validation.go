...
func ValidatePodPresetSpec(spec *settings.PodPresetSpec, fldPath *field.Path) field.ErrorList {
	...
	_, vErrs := apivalidation.ValidateVolumes(spec.Volumes, fldPath.Child("volumes"))
	...
	allErrs = append(allErrs, apivalidation.ValidateVolumeMounts(spec.VolumeMounts, spec.Volumes, nil, fldPath.Child("volumeMounts"))...)
