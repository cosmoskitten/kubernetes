...
func ValidatePodSecurityPolicySpec(spec *extensions.PodSecurityPolicySpec, fldPath *field.Path) field.ErrorList {
	...
	allErrs = append(allErrs, validatePSPAllowedHostPath(fldPath.Child("allowedHostPaths"), spec.AllowedHostPaths)...)
...
// validatePSPAllowedHostPath validates the allowed host paths of the PodSecurityPolicy.
func validatePSPAllowedHostPath(fldPath *field.Path, paths []string) field.ErrorList {
	allErrs := field.ErrorList{}

	for _, path := range paths {
		if !filepath.IsAbs(path) {
			allErrs = append(allErrs, field.Invalid(fldPath.Child("allowedHostPaths"), path, "must specify a absolute path"))
		}
	}
	return allErrs
}
