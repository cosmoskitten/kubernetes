...
func controlledSSHistories(c externalclientset.Interface, namespace, name string) (*appsv1beta1.StatefulSet, []*appsv1beta1.ControllerRevision, error) {
	ss, err := c.AppsV1beta1().StatefulSets(namespace).Get(name, metav1.GetOptions{})
	if err != nil {
	...
	historyList, err := c.AppsV1beta1().ControllerRevisions(ss.Namespace).List(metav1.ListOptions{LabelSelector: selector.String()})
	if err != nil {
		return nil, nil, err
	}
	for i := range historyList.Items {
