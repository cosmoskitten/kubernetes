...
func (h *DaemonSetHistoryViewer) ViewHistory(namespace, name string, revision int64) (string, error) {
	...
	allHistory, err := controlledHistories(versionedAppsClient, versionedDS.Spec.Selector, namespace, versionedDS.UID)
	if err != nil {
...
func controlledHistories(apps clientappsv1beta1.AppsV1beta1Interface, labelSelector *metav1.LabelSelector, namespace string, uid types.UID) ([]*appsv1beta1.ControllerRevision, error) {
	...
	for i := range historyList.Items {
		...
		// Skip history that doesn't belong to the API object
		if controllerRef := controller.GetControllerOf(&history); controllerRef == nil || controllerRef.UID != uid {
			continue
		}
