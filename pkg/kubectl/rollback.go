...
func (r *StatefulSetRollbacker) Rollback(obj runtime.Object, updatedAnnotations map[string]string, toRevision int64, dryRun bool) (string, error) {
	...
	versionedClient := versionedClientsetForStatefulSet(r.c)
	versionedSS, allHistory, err := controlledSSHistories(versionedClient, ss.Namespace, ss.Name)
	if err != nil {
	...
	if dryRun {
		...
		w := printersinternal.NewPrefixWriter(content)
		internalTemplate := &api.PodTemplateSpec{}
		if err := k8s_api_v1.Convert_v1_PodTemplateSpec_To_api_PodTemplateSpec(&appliedSS.Spec.Template, internalTemplate, nil); err != nil {
			return "", fmt.Errorf("failed to convert podtemplate while printing: %v", err)
		}
		printersinternal.DescribePodTemplate(internalTemplate, w)
		return fmt.Sprintf("will roll back to %s", content.String()), nil
	...
	// Restore revision
	if _, err = versionedClient.AppsV1beta1().StatefulSets(ss.Namespace).Patch(ss.Name, types.StrategicMergePatchType, toHistory.Data.Raw); err != nil {
		return "", fmt.Errorf("failed restoring revision %d: %v", toRevision, err)
	}
