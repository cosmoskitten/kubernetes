...
// RemoveAnnotations remove change-cause annotation in input runtime object
func RemoveAnnotations(obj runtime.Object) error {
	accessor, err := meta.Accessor(obj)
	if err != nil {
		return err
	}
...
// RemoveAnnotationsPatch craetes a patch between the origin input resource info
// and the input resource info without change-cause annotation
func RemoveAnnotationsPatch(info *resource.Info) ([]byte, types.PatchType, error) {
	// Get a versioned object
	obj, err := info.Mapping.ConvertToVersion(info.Object, info.Mapping.GroupVersionKind.GroupVersion())
	if err != nil {
		return nil, types.StrategicMergePatchType, err
