...
func AddIncludeUninitializedFlag(cmd *cobra.Command) {
	cmd.Flags().Bool(IncludeUninitializedFlag, false, `If true, this flag overrides the flags that make the kubectl commands apply to uninitialized objects, e.g., "--all" and "--selector". Only objects with empty metadata.initializers are regarded as initialized.`)
}
