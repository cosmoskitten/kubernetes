...
func TestCreatePersistentVolumeClaim(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"test-duplicate-accessmodes": {
			accessmode: "ReadWriteOnce,RWO,ROX",
			capacity:   "1Gi",
			expectedPVC: &api.PersistentVolumeClaim{
				ObjectMeta: v1.ObjectMeta{
					Name: pvcName,
	...
	for name, test := range tests {
		...
		if test.volumeName != "" {
			cmd.Flags().Set("volume-name", test.volumeName)
		}
		...
		if expectedPVC.Spec.VolumeName != realPVC.Spec.VolumeName {
			t.Errorf("%s:\nexpected volume name:\n%s\nsaw volume name:\n%s", name, expectedPVC.Spec.VolumeName, realPVC.Spec.VolumeName)
		}
