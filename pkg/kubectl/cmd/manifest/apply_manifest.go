...
// NewCmdApplyManifest creates a new apply-manifest command.
func NewCmdApplyManifest(f cmdutil.Factory, out, errOut io.Writer) *cobra.Command {
	var options applyManifestOptions

	cmd := &cobra.Command{
		Use:   "apply-manifest [path]",
		Short: "Use a Manifest file to apply a set of configurations",
		Long:  templates.LongDesc("Use a Manifest file to apply a set of configurations"),
		Example: templates.Examples(`
		# Use the Kube-manifest.yaml file under somedir/ to apply a set of configurations.
		kubectl alpha apply-manifest -f somedir/`),
		Run: func(cmd *cobra.Command, args []string) {
			cmdutil.CheckErr(Validate(cmd, args))
			cmdutil.CheckErr(Complete(cmd, args))
			cmdutil.CheckErr(RunApplyManifest(f, cmd, out, errOut, &options))
		},
	}
...
// Validate validates apply-manifest command.
func Validate(cmd *cobra.Command, args []string) error {
	return nil
}

// Complete completes apply-manifest command.
func Complete(cmd *cobra.Command, args []string) error {
	return nil
}

// RunApplyManifest runs apply-manifest command (do real work).
func RunApplyManifest(f cmdutil.Factory, cmd *cobra.Command, out, errOut io.Writer, options *applyManifestOptions) error {
