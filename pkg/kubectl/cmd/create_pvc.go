...
var (
	...
	pvcExample = templates.Examples(i18n.T(`
		...
		# Note: in this case, the created PersistentVolumeClaim will use an exact PersistentVolume which should be created manually by user. This will bypass dynamic provisioning and common binding procedure.
		kubectl create persistentvolumeclaim myclaim --access-mode=ReadWriteOnce --capacity=1Gi --volume-name=highqualityvolume
...
func NewCmdCreatePersistentVolumeClaim(f cmdutil.Factory, cmdOut io.Writer) *cobra.Command {
	...
	cmd.Flags().StringVar(&c.VolumeName, "volume-name", "", "Name of PersistentVolume that should be bound to the claim. If empty (=default), Kubernetes finds the best matching volume or even provisions a new one automatically if dynamic provisioning is available.")
