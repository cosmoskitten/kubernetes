...
var (
	...
	pvcExample = templates.Examples(i18n.T(`
		...
		kubectl create persistentvolumeclaim myclaim --access-mode=ReadWriteOnce --capacity=1Gi --storage-class=HightQuality

		# Create a persistent volume claim named "myclaim" with "ReadWriteOnce" access mode, "1Gi" capacity, requiring a persistent volume named "HighQualityVolume".
		kubectl create persistentvolumeclaim myclaim --access-mode=ReadWriteOnce --capacity=1Gi --volume-name=HightQualityVolume
...
// PersistentVolumeClaim is a command to ease creating PersistentVolumeClaims.
func NewCmdCreatePersistentVolumeClaim(f cmdutil.Factory, cmdOut io.Writer) *cobra.Command {
	c := &CreatePVCOptions{
		Out: cmdOut,
	}
	cmd := &cobra.Command{
		Use:     "persistentvolumeclaim NAME --access-mode=accessmode --capacity=capacity [--storage-class=storageclass] [--volume-name=volumename] [--dry-run]",
		Short:   pvcLong,
		Long:    pvcLong,
		Example: pvcExample,
	...
	cmd.Flags().StringSlice("access-mode", []string{}, "AccessMode contains the desired access modes the volume should have.")
	...
	cmd.Flags().StringVar(&c.VolumeName, "volume-name", "", "VolumeName is the binding reference to the PersistentVolume backing this claim.")
...
func (c *CreatePVCOptions) Complete(f cmdutil.Factory, cmd *cobra.Command, args []string) error {
	...
	// Transform access mode abbreviation to complete access mode and remove duplicate access mode.
	accessModes := []api.PersistentVolumeAccessMode{}
	modes := cmdutil.GetFlagStringSlice(cmd, "access-mode")
	for _, m := range modes {
		var accessMode api.PersistentVolumeAccessMode
		switch m {
		case "RWX", string(api.ReadWriteMany):
			accessMode = api.ReadWriteMany
		case "RWO", string(api.ReadWriteOnce):
			accessMode = api.ReadWriteOnce
		case "ROX", string(api.ReadOnlyMany):
			accessMode = api.ReadOnlyMany
		default:
			return fmt.Errorf("invalid access mode: %s", m)
		}
	...
	storageClassFlag := cmd.Flags().Lookup("storage-class")
	if storageClassFlag != nil && storageClassFlag.Changed {
...
func (c *CreatePVCOptions) RunCreatePVC() error {
	...
	pvc.Spec.VolumeName = c.VolumeName
	...
	if useShortOutput := c.OutputFormat == "name"; useShortOutput || len(c.OutputFormat) == 0 {
		cmdutil.PrintSuccess(c.Mapper, useShortOutput, c.Out, "persistentvolumeclaims", c.Name, c.DryRun, "created")
		return nil
	}
