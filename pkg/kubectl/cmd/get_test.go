...
func TestGetUnknownSchemaObject(t *testing.T) {
	...
	for i, obj := range actual {
		...
		actualJSON := runtime.EncodeOrDie(globalscheme.Codecs.LegacyCodec(), obj)
	...
	for i, obj := range actual {
		...
		case runtime.Unstructured, *runtime.Unknown:
			actualObj, err = runtime.Decode(
				globalscheme.Codecs.UniversalDecoder(),
				[]byte(runtime.EncodeOrDie(globalscheme.Codecs.LegacyCodec(), obj)))
		default:
			actualObj = obj
			err = nil
		}
...
func TestGetMixedGenericObjects(t *testing.T) {
	...
	fn := func(obj runtime.Object) unstructured.Unstructured {
		data, err := runtime.Encode(globalscheme.Codecs.LegacyCodec(schema.GroupVersion{Version: "v1"}), obj)
		if err != nil {
			panic(err)
		}
...
func TestGetMultipleTypeObjectsWithSelector(t *testing.T) {
	...
	tf.UnstructuredClient = &fake.RESTClient{
		...
		NegotiatedSerializer: unstructuredSerializer,
		Client: fake.CreateHTTPClient(func(req *http.Request) (*http.Response, error) {
			if req.URL.Query().Get(metav1.LabelSelectorQueryParam(globalscheme.Registry.GroupOrDie(api.GroupName).GroupVersion.String())) != "a=b" {
				t.Fatalf("request url: %#v,and request: %#v", req.URL, req)
			}
			switch req.URL.Path {
			case "/namespaces/test/pods":
