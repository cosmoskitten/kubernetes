...
func NewCmdDelete(f cmdutil.Factory, out, errOut io.Writer) *cobra.Command {
	...
	cmd.Flags().Bool("wait", true, "If true, waits for the resource to be deleted before returning. True by default, expect for namespace deletion and pod graceful deletion")
