...
func NewCmdDelete(f cmdutil.Factory, out, errOut io.Writer) *cobra.Command {
	...
	cmd.Flags().BoolVar(&options.DeleteAll, "all", false, "Select all resources (include the uninitialized objects by default, unless user explicitly set --include-uninitialized=false) in the namespace of the specified resource types.")
