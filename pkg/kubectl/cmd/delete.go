...
func NewCmdDelete(f cmdutil.Factory, out, errOut io.Writer) *cobra.Command {
	...
	cmd.Flags().BoolVar(&options.DeleteAll, "all", false, "Delete all resources in the namespace of the specified resource types. It will delete the uninitialized objects as well, unless --include-uninitialized=false is set explicitly.")
...
func (o *DeleteOptions) Complete(f cmdutil.Factory, out, errOut io.Writer, args []string, cmd *cobra.Command) error {
	...
	o.Mapper = mapper
	includeUninitialized := cmdutil.GetIncludeUninitialized(cmd, o.DeleteAll, "")
	r := builder.
		ContinueOnError().
