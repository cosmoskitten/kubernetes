...
func (o *RBACRuleOptions) GetNewRules(f cmdutil.Factory) ([]rbac.PolicyRule, error) {
...
func updatePolicyRuleForObject(obj runtime.Object, targetRule *rbac.PolicyRule, fn updatePolicyRule) error {
	...
	switch t := obj.(type) {
	case *rbac.Role:
		t.Rules, err = fn(breakdownRules(t.Rules), targetRule)
		return err
	case *rbac.ClusterRole:
		t.Rules, err = fn(breakdownRules(t.Rules), targetRule)
		return err
...
func addPolicyRule(existingRules []rbac.PolicyRule, targetRule *rbac.PolicyRule) ([]rbac.PolicyRule, error) {
	rules := []rbac.PolicyRule{}
	rules = append(existingRules, *targetRule)
...
func breakdownRules(originalRules []rbac.PolicyRule) []rbac.PolicyRule {
	rules := []rbac.PolicyRule{}
