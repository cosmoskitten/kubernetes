...
func TestServiceAccountRemote(t *testing.T) {
	inputs := []struct {
	...
	}{
		{
			object: &extensions.ReplicaSet{
				TypeMeta:   metav1.TypeMeta{Kind: "ReplicaSet", APIVersion: globalscheme.Registry.GroupOrDie(extensions.GroupName).GroupVersion.String()},
		...
		},
		{
			object: &extensions.DaemonSet{
				TypeMeta:   metav1.TypeMeta{Kind: "DaemonSet", APIVersion: globalscheme.Registry.GroupOrDie(extensions.GroupName).GroupVersion.String()},
				ObjectMeta: metav1.ObjectMeta{Name: "nginx"},
			},
			apiPrefix: "/apis", apiGroup: extensions.GroupName,
		...
		},
		{
			object: &api.ReplicationController{
				TypeMeta:   metav1.TypeMeta{Kind: "ReplicationController", APIVersion: globalscheme.Registry.GroupOrDie(api.GroupName).GroupVersion.String()},
				ObjectMeta: metav1.ObjectMeta{Name: "nginx"},
			},
			...
			args: []string{"replicationcontroller", "nginx", serviceAccount}},
		{
			object: &extensions.Deployment{
				TypeMeta:   metav1.TypeMeta{Kind: "Deployment", APIVersion: globalscheme.Registry.GroupOrDie(extensions.GroupName).GroupVersion.String()},
				ObjectMeta: metav1.ObjectMeta{Name: "nginx"},
			},
			apiPrefix: "/apis", apiGroup: extensions.GroupName,
		...
		{
			object: &batch.Job{
				TypeMeta:   metav1.TypeMeta{Kind: "Job", APIVersion: globalscheme.Registry.GroupOrDie(batch.GroupName).GroupVersion.String()},
				ObjectMeta: metav1.ObjectMeta{Name: "nginx"},
			},
		...
		},
		{
			object: &apps.StatefulSet{
				TypeMeta:   metav1.TypeMeta{Kind: "StatefulSet", APIVersion: globalscheme.Registry.GroupOrDie(apps.GroupName).GroupVersion.String()},
				ObjectMeta: metav1.ObjectMeta{Name: "nginx"},
			},
	...
	for _, input := range inputs {
		...
		groupVersion := globalscheme.Registry.GroupOrDie(input.apiGroup).GroupVersion
		testapi.Default = testapi.Groups[input.apiGroup]
