...
func TestHelperList(t *testing.T) {
	...
	for _, test := range tests {
		...
		obj, err := modifier.List("bar", api.Registry.GroupOrDie(api.GroupName).GroupVersion.String(), labels.SelectorFromSet(labels.Set{"foo": "baz"}), fields.SelectorFromSet(fields.Set{}), false)
		if (err != nil) != test.Err {
			t.Errorf("unexpected error: %t %v", test.Err, err)
		}
		if err != nil {
...
func TestHelperListSelectorCombination(t *testing.T) {
	...
	for _, test := range tests {
		_, err := modifier.List("bar", api.Registry.GroupOrDie(api.GroupName).GroupVersion.String(), test.LabelSelector, test.FieldSelector, false)
		if test.Err {
			if err == nil {
