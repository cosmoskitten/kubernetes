...
func (m *Helper) List(namespace, apiVersion string, labelSelector labels.Selector, fieldSelector string, export, includeUninitialized bool) (runtime.Object, error) {
	if labelSelector != nil && !labelSelector.Empty() && len(fieldSelector) > 0 {
...
func (m *Helper) Watch(namespace, resourceVersion, apiVersion string, labelSelector labels.Selector, fieldSelector string) (watch.Interface, error) {
	if labelSelector != nil && !labelSelector.Empty() && len(fieldSelector) > 0 {
		return nil, fmt.Errorf("cannot combine label selector and field selector together")
	}
