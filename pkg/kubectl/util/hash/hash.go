...
func encodeConfigMap(cm *api.ConfigMap) (string, error) {
	...
	return fmt.Sprintf(`{"data":%s,"kind":"ConfigMap","name":"%s"}`, string(data), cm.Name), nil
}
...
func encodeSecret(sec *api.Secret) (string, error) {
	...
	return fmt.Sprintf(`{"data":%s,"kind":"Secret","name":"%s","type":"%s"}`, string(data), sec.Name, string(sec.Type)), nil
}
