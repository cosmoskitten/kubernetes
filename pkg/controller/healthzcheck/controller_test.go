...
func TestHTTPSHealthzChecker(t *testing.T) {
	...
	if ok, err := checker.Check(serverHost, ports); err != nil {
		t.Errorf("expected HTTPS check to succeed on valid healthz server, but got error: %v", err)
	} else if !ok {
		t.Errorf("expected HTTPS check to succeed on valid healthz server, but it failed")
	}

	checker = NewHTTPSHealthzChecker(httpsClient, "somethingelse")
	if ok, err := checker.Check(serverHost, ports); err != nil {
		t.Errorf("expected HTTPS check to fail on a 404 status code, but got error: %v", err)
	} else if ok {
		t.Errorf("expected HTTPS check to fail on a 404 status code, but it succeeded")
	}
	...
	checker = NewHTTPSHealthzChecker(httpsClient, "healthz")
	if ok, err := checker.Check(serverHost, ports); err != nil {
		t.Errorf("expected HTTPS check to fail on a non-existant HTTP server, but got error: %v", err)
	} else if ok {
		t.Errorf("expected HTTPS check to fail on a non-existant HTTP server, but it succeeded")
	}

}

func TestHTTPHealthzChecker(t *testing.T) {
	httpServer := httptest.NewServer(http.HandlerFunc(func(respWriter http.ResponseWriter, req *http.Request) {
		if req.URL.Path == "/healthz" {
			fmt.Fprintln(respWriter, "ok")
			return
		}
	...
	checker := NewHTTPSHealthzChecker(httpsClient, "healthz")
	if ok, err := checker.Check(serverHost, ports); err != nil {
		t.Errorf("expected HTTP check to succeed on valid healthz server, but got error: %v", err)
	} else if !ok {
		t.Errorf("expected HTTP check to succeed on valid healthz server, but it failed")
	}

	checker = NewHTTPSHealthzChecker(httpsClient, "somethingelse")
	if ok, err := checker.Check(serverHost, ports); err != nil {
		t.Errorf("expected HTTP check to fail on a 404 status code, but got error: %v", err)
	} else if ok {
		t.Errorf("expected HTTP check to fail on a 404 status code, but it succeeded")
	}
	...
	checker = NewHTTPSHealthzChecker(httpsClient, "healthz")
	if ok, err := checker.Check(serverHost, ports); err != nil {
		t.Errorf("expected HTTP check to fail on a non-existant HTTP server, but got error: %v", err)
	} else if ok {
		t.Errorf("expected HTTP check to fail on a non-existant HTTP server, but it succeeded")
	}
