...
func (pvlc *PersistentVolumeLabelController) addLabels(key string) error {
	...
	if needsInitialization(volume.Initializers, initializerName) {
		...
		return pvlc.updateVolume(volume, volumeLabels)
	}
...
func (pvlc *PersistentVolumeLabelController) updateVolume(vol *v1.PersistentVolume, volLabels map[string]string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to marshal old persistentvolume %#v for persistentvolume %q: %v", vol, volName, err)
	}
