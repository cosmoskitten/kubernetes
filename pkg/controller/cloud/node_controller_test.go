	...
	for _, tc := range testCases {
		t.Run(tc.testName, func(t *testing.T) {
			...
			assert.EqualValues(t, tc.expectedCalls, fc.Calls,
				"expected cloud provider methods `%v` to be called but `%v` was called ", tc.expectedCalls, fc.Calls)

			assert.False(t, tc.existsByProviderID && tc.existsByProviderID != exists,
				"expected exist by provider id to be `%t` but got `%t`", tc.existsByProviderID, exists)
...
func TestZoneInitialized(t *testing.T) {
	...
	assert.Equal(t, 1, len(fnh.UpdatedNodes), "Node was not updated")
	assert.Equal(t, "node0", fnh.UpdatedNodes[0].Name, "Node was not updated")
	assert.Equal(t, 2, len(fnh.UpdatedNodes[0].ObjectMeta.Labels), "Node label for Region and Zone were not set")
	assert.Equal(t, "us-west", fnh.UpdatedNodes[0].ObjectMeta.Labels[kubeletapis.LabelZoneRegion], "Node Region not correctly updated")
	assert.Equal(t, "us-west-1a", fnh.UpdatedNodes[0].ObjectMeta.Labels[kubeletapis.LabelZoneFailureDomain], "Node FailureDomain not correctly updated")
}
...
func TestNodeAddressesChangeDetected(t *testing.T) {
	...
	assert.False(t, nodeAddressesChangeDetected(addressSet1, addressSet2), "Node address changes are not detected correctly")
	...
	assert.True(t, nodeAddressesChangeDetected(addressSet1, addressSet2), "Node address changes are not detected correctly")
