...
func TestNodeAddressesNotUpdate(t *testing.T) {
	...
	if len(fnh.UpdatedNodes) != 1 && fnh.UpdatedNodes[0].Name != "node0" {
		t.Errorf("Node was not correctly updated, got: %v, wanted: the only one node and the name is \"node0\")", fnh.UpdatedNodes)
	}

	if len(fnh.UpdatedNodes[0].Status.Addresses) != 0 {
		t.Errorf("Node addresses was not correctly updated, the len(address) got=%d, wanted=0", len(fnh.UpdatedNodes[0].Status.Addresses))
	}
}
