...
func NewPersistentVolumeLabelController(
	...
	//	pvlc.pvSynced = pvlc.pvlController.HasSynced
...
func (pvlc *PersistentVolumeLabelController) updateVolume(volume *v1.PersistentVolume) error {
	//func (pvlc *PersistentVolumeLabelController) updateVolume(labels map[string]string) error {
	...
	err := clientretry.RetryOnConflict(updatePVLabelsBackoff, func() error {
		...
		if err != nil {
			return fmt.Errorf("failed to marshal old persistentvolume %#v for persistentvolume %q: %v", curVol, volume.Name, err)
		}

		objCopy, err := scheme.Scheme.DeepCopy(curVol)
		if err != nil {
			return fmt.Errorf("failed to copy persistentvolume object %#v: %v", curVol, err)
		}
		newVolume, ok := (objCopy).(*v1.PersistentVolume)
		if !ok {
			return fmt.Errorf("failed to cast copy into persistentvolume object %#v: %v", newVolume, err)
		}
		newVolume.Labels = volume.Labels
		pvlc.removeInitializer(newVolume)

		newData, err := json.Marshal(newVolume)
		if err != nil {
			return fmt.Errorf("failed to marshal new persistentvolume %#v for persistentvolume %q: %v", newVolume, volume.Name, err)
		}

		patchBytes, err := strategicpatch.CreateTwoWayMergePatch(oldData, newData, v1.PersistentVolume{})
		if err != nil {
			return fmt.Errorf("failed to create patch for persistentvolume %q: %v", volume.Name, err)
		}

		_, err = pvlc.kubeClient.Core().PersistentVolumes().Patch(string(volume.Name), types.StrategicMergePatchType, patchBytes)
		return err
	})
	if err == nil {
