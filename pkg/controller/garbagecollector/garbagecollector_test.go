...
func TestGarbageCollectorConstruction(t *testing.T) {
	...
	rm := &testRESTMapper{meta.MultiRESTMapper{tweakableRM, globalscheme.Registry.RESTMapper()}}
...
func setupGC(t *testing.T, config *restclient.Config) garbageCollector {
	config.ContentConfig.NegotiatedSerializer = serializer.DirectCodecFactory{CodecFactory: metaonly.NewMetadataCodecFactory()}
	metaOnlyClientPool := dynamic.NewClientPool(config, globalscheme.Registry.RESTMapper(), dynamic.LegacyAPIPathResolverFunc)
	config.ContentConfig.NegotiatedSerializer = nil
	clientPool := dynamic.NewClientPool(config, globalscheme.Registry.RESTMapper(), dynamic.LegacyAPIPathResolverFunc)
	podResource := map[schema.GroupVersionResource]struct{}{{Version: "v1", Resource: "pods"}: {}}
	client := fake.NewSimpleClientset()
	sharedInformers := informers.NewSharedInformerFactory(client, 0)
	alwaysStarted := make(chan struct{})
	close(alwaysStarted)
	gc, err := NewGarbageCollector(metaOnlyClientPool, clientPool, &testRESTMapper{globalscheme.Registry.RESTMapper()}, podResource, ignoredResources, sharedInformers, alwaysStarted)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGCListWatcher(t *testing.T) {
	...
	srv, clientConfig := testServerAndClientConfig(testHandler.ServeHTTP)
	defer srv.Close()
	clientPool := dynamic.NewClientPool(clientConfig, globalscheme.Registry.RESTMapper(), dynamic.LegacyAPIPathResolverFunc)
