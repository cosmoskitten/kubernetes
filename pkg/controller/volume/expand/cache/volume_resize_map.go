...
func (resizeMap *volumeResizeMap) MarkAsResized(pvcr *PvcWithResizeRequest) error {
	...
	emptyCondition := []v1.PvcCondition{}

	err = resizeMap.updatePvcCapacityAndConditions(pvcr, pvcr.ExpectedSize, emptyCondition)
	if err != nil {
		glog.V(4).Infof("Error updating PV spec capacity for volume %q with : %v", pvcr.QualifiedName(), err)
		resizeMap.addBackResizeRequest(pvcr)
		return err
...
func (resizeMap *volumeResizeMap) updatePvcCapacityAndConditions(pvcr *PvcWithResizeRequest, newSize resource.Quantity, pvcConditions []v1.PvcCondition) error {
