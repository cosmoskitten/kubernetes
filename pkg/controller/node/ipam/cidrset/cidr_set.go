...
// TODO: Replace this with math/bits when upgrading to go 1.9
	...
	case s.clusterIP.To16() != nil:
		{
			...
			const halfIPv6Len = net.IPv6len / 2
			...
			if s.subNetMaskSize <= halfV6NBits {
				...
				if s.clusterMaskSize < halfV6NBits {
					...
					if indexMaxBit > btl {
						leftIndex := uint64(index) >> btl
						leftClusterIP |= leftIndex
					}
					rightClusterIP |= uint64(index) << uint(v6NBits-s.subNetMaskSize)

				} else {
					// We only care about right side IP
					rightClusterIP |= uint64(index) << uint(s.subNetMaskSize-halfV6NBits)
				}
			}
