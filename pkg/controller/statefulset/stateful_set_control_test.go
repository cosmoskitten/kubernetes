...
func setupController(client clientset.Interface) (*fakeStatefulPodControl, *fakeStatefulSetStatusUpdater, StatefulSetControlInterface, chan struct{}) {
	...
	spc := newFakeStatefulPodControl(informerFactory.Core().V1().Pods(), informerFactory.Apps().V1beta2().StatefulSets())
	ssu := newFakeStatefulSetStatusUpdater(informerFactory.Apps().V1beta2().StatefulSets())
	ssc := NewDefaultStatefulSetControl(spc, ssu, history.NewFakeHistory(informerFactory.Apps().V1beta2().ControllerRevisions()))

	stop := make(chan struct{})
	informerFactory.Start(stop)
	cache.WaitForCacheSync(
		stop,
		informerFactory.Apps().V1beta2().StatefulSets().Informer().HasSynced,
		informerFactory.Core().V1().Pods().Informer().HasSynced,
		informerFactory.Apps().V1beta2().ControllerRevisions().Informer().HasSynced,
	)
...
func TestStatefulSetControl_getSetRevisions(t *testing.T) {
	...
	testFn := func(test *testcase, t *testing.T) {
		...
		spc := newFakeStatefulPodControl(informerFactory.Core().V1().Pods(), informerFactory.Apps().V1beta2().StatefulSets())
		ssu := newFakeStatefulSetStatusUpdater(informerFactory.Apps().V1beta2().StatefulSets())
		ssc := defaultStatefulSetControl{spc, ssu, history.NewFakeHistory(informerFactory.Apps().V1beta2().ControllerRevisions())}
