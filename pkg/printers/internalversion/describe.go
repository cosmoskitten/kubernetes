...
func describeNetworkPolicySpec(nps networking.NetworkPolicySpec, w PrefixWriter) {
	...
	if (nps.PodSelector.MatchLabels == nil || len(nps.PodSelector.MatchLabels) == 0) &&
		(nps.PodSelector.MatchExpressions == nil || len(nps.PodSelector.MatchExpressions) == 0) {
		w.Write(LEVEL_2, "<none> (Allowing all traffic to all pods in this namespace)\n")
	} else {
...
func printNetworkPolicySpecIngressFrom(npirs []networking.NetworkPolicyIngressRule, initialIndent, innerIndent string, w PrefixWriter) {
	if npirs == nil || len(npirs) == 0 {
