...
func AddHandlers(h printers.PrintHandler) {
	...
	nodeColumnDefinitions := []metav1alpha1.TableColumnDefinition{
		...
		{Name: "Role", Type: "string", Description: "The role of the node"},
		{Name: "Age", Type: "string", Description: metav1.ObjectMeta{}.SwaggerDoc()["creationTimestamp"]},
...
func printNode(obj *api.Node, options printers.PrintOptions) ([]metav1alpha1.TableRow, error) {
	...
	roles := findNodeRoles(obj)

	row.Cells = append(row.Cells, obj.Name, strings.Join(status, ","), strings.Join(roles, ","), translateTimestamp(obj.CreationTimestamp), obj.Status.NodeInfo.KubeletVersion)
	if options.Wide {
