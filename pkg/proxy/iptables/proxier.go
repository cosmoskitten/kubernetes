...
type Proxier struct {
	...
	portsMap     map[proxyutil.LocalPort]utilproxy.Closeable
...
func (l *listenPortOpener) OpenLocalPort(lp *proxyutil.LocalPort) (utilproxy.Closeable, error) {
...
func NewProxier(ipt utiliptables.Interface,
	...
	proxier := &Proxier{
		portsMap:                 make(map[proxyutil.LocalPort]utilproxy.Closeable),
...
func (esp *endpointServicePair) IPPart() string {
	return proxyutil.IPPart(esp.endpoint)
}
...
func (proxier *Proxier) syncProxyRules() {
	...
	replacementPortsMap := map[proxyutil.LocalPort]utilproxy.Closeable{}
	...
	for svcName, svcInfo := range proxier.serviceMap {
		...
		args = append(args[:0],
			...
			"-d", proxyutil.HostAddress(svcInfo.clusterIP),
		...
		for _, externalIP := range svcInfo.externalIPs {
			...
			args = append(args[:0],
				...
				"-m", protocol, "-p", protocol,
				"-d", proxyutil.HostAddress(net.ParseIP(externalIP)),
				"--dport", strconv.Itoa(svcInfo.port),
			)
		...
		for _, ingress := range svcInfo.loadBalancerStatus.Ingress {
			if ingress.IP != "" {
				...
				args = append(args[:0],
					...
					"-m", protocol, "-p", protocol,
					"-d", proxyutil.HostAddress(net.ParseIP(ingress.IP)),
					"--dport", strconv.Itoa(svcInfo.port),
				)
				...
				if len(svcInfo.loadBalancerSourceRanges) == 0 {
					...
					if allowFromNode {
						writeLine(proxier.natRules, append(args, "-s", proxyutil.HostAddress(net.ParseIP(ingress.IP)), "-j", string(chosenChain))...)
					}
				}
		...
		if svcInfo.nodePort != 0 {
			...
			if proxier.portsMap[lp] != nil {
				...
				if lp.Protocol == "udp" {
					...
					err := proxyutil.ClearUDPConntrackForPort(proxier.exec, lp.Port, isIPv6)
					if err != nil {
		...
		for i, endpointChain := range endpointChains {
			...
			writeLine(proxier.natRules, append(args,
				"-s", proxyutil.HostAddress(net.ParseIP(epIP)),
...
func openLocalPort(lp *proxyutil.LocalPort) (utilproxy.Closeable, error) {
