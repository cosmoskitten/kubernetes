...
func (proxier *Proxier) syncProxyRules() {
	...
	for svcName, svcInfo := range proxier.serviceMap {
		...
		args = append(args[:0],
			...
			"-d", utilproxy.HostAddress(svcInfo.clusterIP),
		...
		for _, externalIP := range svcInfo.externalIPs {
			...
			args = append(args[:0],
				...
				"-m", protocol, "-p", protocol,
				"-d", utilproxy.HostAddress(net.ParseIP(externalIP)),
				"--dport", strconv.Itoa(svcInfo.port),
			)
		...
		for _, ingress := range svcInfo.loadBalancerStatus.Ingress {
			if ingress.IP != "" {
				...
				args = append(args[:0],
					...
					"-m", protocol, "-p", protocol,
					"-d", utilproxy.HostAddress(net.ParseIP(ingress.IP)),
					"--dport", strconv.Itoa(svcInfo.port),
				)
				...
				if len(svcInfo.loadBalancerSourceRanges) == 0 {
					...
					if allowFromNode {
						writeLine(proxier.natRules, append(args, "-s", utilproxy.HostAddress(net.ParseIP(ingress.IP)), "-j", string(chosenChain))...)
					}
				}
		...
		for i, endpointChain := range endpointChains {
			...
			writeLine(proxier.natRules, append(args,
				"-s", utilproxy.HostAddress(net.ParseIP(epIP)),
