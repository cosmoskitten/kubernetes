...
func TestAlphaVolumeMode(t *testing.T) {
	...
	// Test Successful
	if errs := ValidatePersistentVolume(CreateTestVolModePV(api.PersistentVolumeBlock)); len(errs) != 0 {
		t.Errorf("expected success: %v", errs)
	}
	if errs := ValidatePersistentVolume(CreateTestVolModePV(api.PersistentVolumeFilesystem)); len(errs) != 0 {
		t.Errorf("expected success: %v", errs)
	}
	if errs := ValidatePersistentVolumeClaim(CreateTestVolModePVC(api.PersistentVolumeFilesystem)); len(errs) != 0 {
		t.Errorf("expected success: %v", errs)
	}
	if errs := ValidatePersistentVolumeClaim(CreateTestVolModePVC(api.PersistentVolumeBlock)); len(errs) != 0 {
		t.Errorf("expected success: %v", errs)
	}
	// Test invalid
	if errs := ValidatePersistentVolume(CreateTestVolModePV(api.PersistentVolumeMode("fake"))); len(errs) == 0 {
		t.Errorf("expected failure: %v", errs)
	}
	if errs := ValidatePersistentVolume(CreateTestVolModePV(api.PersistentVolumeMode(""))); len(errs) == 0 {
		t.Errorf("expected failure: %v", errs)
	}
	if errs := ValidatePersistentVolumeClaim(CreateTestVolModePVC(api.PersistentVolumeMode("fake"))); len(errs) == 0 {
		t.Errorf("expected failure: %v", errs)
	}
	if errs := ValidatePersistentVolumeClaim(CreateTestVolModePVC(api.PersistentVolumeMode(""))); len(errs) == 0 {
		t.Errorf("expected failure: %v", errs)
	}
}
...
func TestAlphaValidateVolumeDevices(t *testing.T) {
	...
	if errs := ValidateVolumeDevices(disabledAlphaVolDevice, volumes, field.NewPath("field")); len(errs) == 0 {
		t.Errorf("expected failure: %v", errs)
	}
}
