...
func TestAlphaVolumeMode(t *testing.T){
	...
	validModePV := api.PersistentVolume{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "foo",
			Namespace: "",
		},
	...
	invalidModePV := api.PersistentVolume{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "foo",
			Namespace: "",
		},
	...
	// PersistentVolumeClaim with VolumeMode set (valid and invalid)
	validModePVC := api.PersistentVolumeClaim{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "foo",
			Namespace: "default",
		},
	...
	invalidModePVC := api.PersistentVolumeClaim{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "foo",
			Namespace: "default",
		},
	...
	if errs := ValidatePersistentVolume(&validModePV); len(errs) != 0 {
		t.Errorf("expected success: %v", errs)
	}
	if errs := ValidatePersistentVolumeClaim(&validModePVC); len(errs) != 0 {
		t.Errorf("expected success: %v", errs)
	}
	if errs := ValidatePersistentVolume(&invalidModePV); len(errs) == 0 {
		t.Errorf("expected failure: %v", errs)
	}
	if errs := ValidatePersistentVolumeClaim(&invalidModePVC); len(errs) == 0 {
		t.Errorf("expected failure: %v", errs)
	}


	// Disable alpha feature BlockVolumeSupport
	err1 := utilfeature.DefaultFeatureGate.Set("BlockVolumeSupport=false")
	if err1 != nil {
