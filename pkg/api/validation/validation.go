...
func ValidateVolumes(volumes []api.Volume, fldPath *field.Path) (map[string]bool, field.ErrorList) {
	allErrs := field.ErrorList{}
...
func getMatchedVolume(name string, volumes map[string]bool) bool {
...
func getMatchedDevice(name string, volumes map[string]bool) (bool, bool) {
	for k, v := range volumes {
		if k == name {
			// matched on device name and volume name
			return true, v
		}
	}
	return false, false
...
func ValidateVolumeMounts(mounts []api.VolumeMount, volumes map[string]bool, container *api.Container, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
	...
	for i, mnt := range mounts {
		...
		if !getMatchedVolume(mnt.Name, volumes) {
...
func ValidateVolumeDevices(devices []api.VolumeDevice, volumes map[string]bool, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
	...
	if devices != nil {
		for i, dev := range devices {
			...
			didMatch, isPVC := getMatchedDevice(devName, volumes)
			if len(devName) == 0 {
				allErrs = append(allErrs, field.Required(idxPath.Child("name"), ""))
			}
...
func validateInitContainers(containers, otherContainers []api.Container, deviceVolumes map[string]bool, fldPath *field.Path) field.ErrorList {
...
func validateContainers(containers []api.Container, deviceVolumes map[string]bool, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
