...
func ValidateService(service *api.Service) field.ErrorList {
	...
	if service.Spec.SessionAffinity == api.ServiceAffinityClientIP {
		...
		if service.Spec.SessionAffinityConfig != nil {
			allErrs = append(allErrs, field.Forbidden(specPath.Child("sessionAffinityConfig"), fmt.Sprintf("sessionAffinityConfig must not be set when session affinity is %s", string(api.ServiceAffinityNone))))
		}
	}
