...
func getVolumeSourceStringFromName(name string, volumes []api.Volume) api.VolumeSource {
	nomatchVol := api.VolumeSource{}
	for _, vol := range volumes {
		if vol.Name == name {
			// We have a match on device and volume name
			return vol.VolumeSource
		}
	}
	return nomatchVol
}
...
func ValidateVolumeDevices(devices []api.VolumeDevice, volumes []api.Volume, fldPath *field.Path) field.ErrorList {
	...
	// pvcvolumes := getClaimVolumesList(volumes)
	if devices != nil {
		for i, dev := range devices {
			idxPath := fldPath.Index(i)
			devName := dev.Name
			devPath := dev.DevicePath
			volSource := getVolumeSourceStringFromName(devName, volumes)
			if len(devName) == 0 {
				allErrs = append(allErrs, field.Required(idxPath.Child("name"), ""))
			}
			...
			// if !pvcvolumes.Has(devName) {
			//	allErrs = append(allErrs, field.Required(idxPath.Child("name"), "volumeDevice (block devices) must have a corresponding PersistentVolumeClaim volume source type for this volume name"))
			// }
			// Must be PersistentVolumeClaim volume source
