...
func validateLimitRangeResourceName(limitType api.LimitType, value string, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
	if value == string(api.ResourceStorageScratch) &&
		!utilfeature.DefaultFeatureGate.Enabled(features.LocalStorageCapacityIsolation) {
		return append(allErrs, field.Forbidden(fldPath, "ResourceStorageScratch field disabled by feature-gate for Resource LimitRange"))
	}
	switch limitType {
