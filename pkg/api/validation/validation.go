...
func ValidateVolumeMounts(mounts []api.VolumeMount, volumes []api.Volume, container *api.Container, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
	...
	for i, mnt := range mounts {
		...
		_, didMatch := getVolumeSourceFromName(mnt.Name, volumes)
		if !didMatch {
...
func validateInitContainers(containers, otherContainers []api.Container, deviceVolumes []api.Volume, fldPath *field.Path) field.ErrorList {
...
func validateContainers(containers []api.Container, deviceVolumes []api.Volume, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
...
func ValidatePodSpec(spec *api.PodSpec, fldPath *field.Path) field.ErrorList {
	...
	_, vErrs := ValidateVolumes(spec.Volumes, fldPath.Child("volumes"))
	allErrs = append(allErrs, vErrs...)
	allErrs = append(allErrs, validateContainers(spec.Containers, spec.Volumes, fldPath.Child("containers"))...)
	allErrs = append(allErrs, validateInitContainers(spec.InitContainers, spec.Containers, spec.Volumes, fldPath.Child("initContainers"))...)
