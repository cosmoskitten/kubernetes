...
func ValidatePersistentVolume(pv *api.PersistentVolume) field.ErrorList {
	...
	if !utilfeature.DefaultFeatureGate.Enabled(features.BlockVolumeSupport) {
		allErrs = append(allErrs, field.Forbidden(specPath.Child("volumeMode"), "volumeMode is disabled by feature-gate"))
		return allErrs
	}
...
func ValidatePersistentVolumeClaimSpec(spec *api.PersistentVolumeClaimSpec, fldPath *field.Path) field.ErrorList {
	...
	if !utilfeature.DefaultFeatureGate.Enabled(features.BlockVolumeSupport) {
		allErrs = append(allErrs, field.Forbidden(fldPath.Child("volumeMode"), "volumeMode is disabled by feature-gate"))
		return allErrs
	}
