...
func getClaimVolumesList(volumes []api.Volume) sets.String {
	allNames := sets.String{}
...
func getVolumeSourceFromName(name string, volumes []api.Volume) (api.VolumeSource, bool) {
	nomatchVol := api.VolumeSource{}
	for _, vol := range volumes {
		if vol.Name == name {
			// We have a match on device and volume name
			return vol.VolumeSource, true
		}
	}
	return nomatchVol, false
}
...
func ValidateVolumeDevices(devices []api.VolumeDevice, volumes []api.Volume, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
	...
	if devices != nil {
		for i, dev := range devices {
			...
			volSource, didMatch := getVolumeSourceFromName(devName, volumes)
			if len(devName) == 0 {
				allErrs = append(allErrs, field.Required(idxPath.Child("name"), ""))
			}
			...
			if didMatch && volSource.PersistentVolumeClaim == nil {
				allErrs = append(allErrs, field.Invalid(idxPath.Child("name"), devName, "volumeDevice does have a matching volume name however, volumeDevices can only use volume source type of PersistentVolumeClaim (pvc)"))
			}
			if !didMatch && volSource.PersistentVolumeClaim == nil {
				allErrs = append(allErrs, field.Invalid(idxPath.Child("name"), devName, "volumeDevice does not have a matching volume name, it is required to have a corresponding pod volume of type PersistentVolumeClaim (pvc) for each volumeDevice"))
			}
			if len(devPath) == 0 {
...
func validateInitContainers(containers, otherContainers []api.Container, volumes sets.String, deviceVolumes []api.Volume, fldPath *field.Path) field.ErrorList {
...
func validateContainers(containers []api.Container, volumes sets.String, deviceVolumes []api.Volume, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
...
func ValidatePodSpec(spec *api.PodSpec, fldPath *field.Path) field.ErrorList {
	...
	allErrs = append(allErrs, validateContainers(spec.Containers, allVolumes, spec.Volumes, fldPath.Child("containers"))...)
	allErrs = append(allErrs, validateInitContainers(spec.InitContainers, spec.Containers, allVolumes, spec.Volumes, fldPath.Child("initContainers"))...)
