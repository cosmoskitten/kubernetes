...
func TestSetDefaults_Affinity(t *testing.T) {
	tests := []struct {
	...
	}{
		{
					...
					Affinity: &v1.Affinity{
						PodAffinity: &v1.PodAffinity{
							RequiredDuringSchedulingIgnoredDuringExecution: []v1.PodAffinityTerm{
								{MaxPodsPerTopologyKey: 2},
								{MaxPodsPerTopologyKey: 1},
								{MaxPodsPerTopologyKey: 0},
							},
						},
					},
				},
			...
			maxPodsPerTopologyKey: []int32{},
			name: "The default value of affinity's MaxPodsPerTopologyKey is 1",
		},
		{
			pod: &v1.Pod{
				Spec: v1.PodSpec{
			...
			maxPodsPerTopologyKey: []int32{2, 1, 1},
			name: "The valid value of anti-affinity's MaxPodsPerTopologyKey is not changed, others is set to 1",
		},
	}
	...
	for _, test := range tests {
		...
		if pod2.Spec.Affinity.PodAffinity != nil {
			...
			for _, term := range affinityTerms {
				if term.MaxPodsPerTopologyKey != 1 {
					t.Errorf("%s: expected 1 for affinity terms, got %d", test.name, term.MaxPodsPerTopologyKey)
				}
			}
		}
		...
		if pod2.Spec.Affinity.PodAntiAffinity != nil {
			...
			if len(antiAffinitiyTerms) != len(test.maxPodsPerTopologyKey) {
				t.Errorf("%s is invalid case: different length of terms", test.name)
				continue
			}

			for i := range antiAffinitiyTerms {
				if antiAffinitiyTerms[i].MaxPodsPerTopologyKey != test.maxPodsPerTopologyKey[i] {
					t.Errorf("%s (%d): expected %d for affinity terms, got %d", test.name,
						i, test.maxPodsPerTopologyKey[i], antiAffinitiyTerms[i].MaxPodsPerTopologyKey)
				}
			}
		}
	}
