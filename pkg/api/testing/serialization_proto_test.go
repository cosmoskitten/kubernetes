...
func TestUniversalDeserializer(t *testing.T) {
	...
	d := globalscheme.Codecs.UniversalDeserializer()
	for _, mediaType := range []string{"application/json", "application/yaml", "application/vnd.kubernetes.protobuf"} {
		info, ok := runtime.SerializerInfoForMediaType(globalscheme.Codecs.SupportedMediaTypes(), mediaType)
		if !ok {
			t.Fatal(mediaType)
		}
		buf := &bytes.Buffer{}
...
func TestAllFieldsHaveTags(t *testing.T) {
	for gvk, obj := range globalscheme.Scheme.AllKnownTypes() {
...
func TestProtobufRoundTrip(t *testing.T) {
	obj := &v1.Pod{}
	fuzzer.FuzzerFor(FuzzerFuncs, rand.NewSource(benchmarkSeed), globalscheme.Codecs).Fuzz(obj)
...
func BenchmarkEncodeCodecFromInternalProtobuf(b *testing.B) {
	...
	s := protobuf.NewSerializer(nil, nil, "application/arbitrary.content.type")
	codec := globalscheme.Codecs.EncoderForVersion(s, v1.SchemeGroupVersion)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkDecodeCodecToInternalProtobuf(b *testing.B) {
	items := benchmarkItems(b)
	width := len(items)
	s := protobuf.NewSerializer(globalscheme.Scheme, globalscheme.Scheme, "application/arbitrary.content.type")
	encoder := globalscheme.Codecs.EncoderForVersion(s, v1.SchemeGroupVersion)
	var encoded [][]byte
	for i := range items {
	...
	decoder := globalscheme.Codecs.DecoderToVersion(s, api.SchemeGroupVersion)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
