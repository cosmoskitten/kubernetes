...
func fuzzInternalObject(t *testing.T, forVersion schema.GroupVersion, item runtime.Object, seed int64) runtime.Object {
	fuzzer.FuzzerFor(FuzzerFuncs, rand.NewSource(seed), globalscheme.Codecs).Fuzz(item)
...
func TestSetControllerConversion(t *testing.T) {
	if err := globalscheme.Scheme.AddConversionFuncs(Convert_v1beta1_ReplicaSet_to_api_ReplicationController); err != nil {
		t.Fatal(err)
	}
	...
	decoder := globalscheme.Codecs.DecoderToVersion(
		globalscheme.Codecs.UniversalDeserializer(),
...
func TestSpecificKind(t *testing.T) {
	...
	// globalscheme.Scheme.Log(t)
	internalGVK := schema.GroupVersionKind{Group: "extensions", Version: runtime.APIVersionInternal, Kind: "DaemonSet"}

	seed := rand.Int63()
	fuzzer := fuzzer.FuzzerFor(FuzzerFuncs, rand.NewSource(seed), globalscheme.Codecs)

	roundtrip.RoundTripSpecificKind(t, internalGVK, globalscheme.Scheme, globalscheme.Codecs, fuzzer, nil)
}
...
func TestCommonKindsRegistered(t *testing.T) {
	for _, kind := range commonKinds {
		for _, group := range testapi.Groups {
			...
			obj, err := globalscheme.Scheme.New(gvk)
			if err != nil {
				t.Error(err)
			}
			defaults := gv.WithKind("")
			var got *schema.GroupVersionKind
			if obj, got, err = globalscheme.Codecs.LegacyCodec().Decode([]byte(`{"kind":"`+kind+`"}`), &defaults, obj); err != nil || gvk != *got {
				t.Errorf("expected %v: %v %v", gvk, got, err)
			}
			data, err := runtime.Encode(globalscheme.Codecs.LegacyCodec(*gv), obj)
			if err != nil {
...
func TestRoundTripTypes(t *testing.T) {
	...
	roundtrip.RoundTripTypes(t, globalscheme.Scheme, globalscheme.Codecs, fuzzer, nonRoundTrippableTypes)
}
...
func TestObjectWatchFraming(t *testing.T) {
	f := fuzzer.FuzzerFor(FuzzerFuncs, rand.NewSource(benchmarkSeed), globalscheme.Codecs)
	secret := &api.Secret{}
	f.Fuzz(secret)
	...
	converted, _ := globalscheme.Scheme.ConvertToVersion(secret, v1.SchemeGroupVersion)
	v1secret := converted.(*v1.Secret)
	for _, info := range globalscheme.Codecs.SupportedMediaTypes() {
		...
		innerDecode := globalscheme.Codecs.DecoderToVersion(embedded, api.SchemeGroupVersion)
...
func benchmarkItems(b *testing.B) []v1.Pod {
	apiObjectFuzzer := fuzzer.FuzzerFor(FuzzerFuncs, rand.NewSource(benchmarkSeed), globalscheme.Codecs)
	items := make([]v1.Pod, 10)
	for i := range items {
		var pod api.Pod
		apiObjectFuzzer.Fuzz(&pod)
		pod.Spec.InitContainers, pod.Status.InitContainerStatuses = nil, nil
		out, err := globalscheme.Scheme.ConvertToVersion(&pod, v1.SchemeGroupVersion)
		if err != nil {
			panic(err)
		}
		items[i] = *out.(*v1.Pod)
...
func BenchmarkEncodeCodecFromInternal(b *testing.B) {
	...
	for i := range items {
		if err := globalscheme.Scheme.Convert(&items[i], &encodable[i], nil); err != nil {
			b.Fatal(err)
		}
	}
	b.ResetTimer()
