...
func (os *OpenStack) getZoneByServer(server *servers.Server) (cloudprovider.Zone, error) {
	az := ""
	var ok bool

	if az, ok = server.Metadata[AvailabilityZone]; !ok {
		return cloudprovider.Zone{}, fmt.Errorf("Can not get %s from the Metadata of instance %s", AvailabilityZone, server.Name)
	}
	if len(az) == 0 {
		return cloudprovider.Zone{}, fmt.Errorf("The %s of instance %s is empty.", AvailabilityZone, server.Name)
	}
