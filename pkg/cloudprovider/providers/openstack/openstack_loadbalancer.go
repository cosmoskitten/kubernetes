...
func getNodeSecurityGroupIDForLB(compute *gophercloud.ServiceClient, nodes []*v1.Node) ([]string, error) {
	...
	for _, node := range nodes {
		...
		for _, securityGroup := range srv.SecurityGroups {
			for _, securityGroupName := range securityGroup {
...
func (lbaas *LbaasV2) ensureSecurityGroup(clusterName string, apiService *v1.Service, nodes []*v1.Node, loadbalancer *loadbalancers.LoadBalancer) error {
	...
	if len(lbaas.opts.NodeSecurityGroupIDs) == 0 {
		...
		if err != nil {
			glog.Errorf("Failed to find node-security-group for loadbalancer service %s/%s: %v", apiService.Namespace, apiService.Name, err)
	...
	if err != nil {
		...
		if ok {
			...
			glog.Errorf("Error occurred finding security group: %s: %v", lbSecGroupName, err)
		...
		for _, port := range ports {
			for _, sourceRange := range sourceRanges.StringSlice() {
				...
				if err != nil {
					...
					return fmt.Errorf("Error parsing source range %s as a CIDR", sourceRange)
				}
	...
	for _, port := range ports {
		for _, nodeSecurityGroupID := range lbaas.opts.NodeSecurityGroupIDs {
			...
			if err != nil {
				return fmt.Errorf("Error occured creating security group for loadbalancer service %s/%s", apiService.Namespace, apiService.Name)
			}
		}
	}
...
func (lbaas *LbaasV2) updateSecurityGroup(clusterName string, apiService *v1.Service, nodes []*v1.Node, loadbalancer *loadbalancers.LoadBalancer) error {
	...
	for _, port := range ports {
		for removal := range removals {
			...
			secGroupRules, err := getSecurityGroupRules(lbaas.network, opts)
			if err != nil && !isNotFound(err) {
				glog.Errorf("Error finding rules for remote group id %s in security group id %s", lbSecGroupID, removal)
				return err
			}
