...
const (
	...
	ServiceAnnotationLoadBalancerInternal = "service.beta.kubernetes.io/openstack-load-balancer-internal"
)
...
func (lbaas *LbaasV2) EnsureLoadBalancer(clusterName string, apiService *v1.Service, nodes []*v1.Node) (*v1.LoadBalancerStatus, error) {
	...
	serviceAnnotationLoadBalancerInternal := getStringFromServiceAnnotation(apiService, ServiceAnnotationLoadBalancerInternal, "true")
	switch serviceAnnotationLoadBalancerInternal {
	case "true":
		internalAnnotation = true
		glog.V(4).Infof("Ensure an internal loadbalancer service with ServiceAnnotationLoadBalancerInternal annotation: %v",
			internalAnnotation)
	case "false":
		internalAnnotation = false
		if len(floatingPool) != 0 {
			glog.V(4).Infof("Ensure an external loadbalancer service with ServiceAnnotationLoadBalancerInternal annotation: %v",
				internalAnnotation)
		} else {
			return nil, fmt.Errorf("floating-network-id or ServiceAnnotationLoadBalancerFloatingNetworkId should be specified when ServiceAnnotationLoadBalancerInternal is false")
		}
	default:
		return nil, fmt.Errorf("unknow ServiceAnnotationLoadBalancerInternal annotation: %v, specify \"true\" or \"false\".",
