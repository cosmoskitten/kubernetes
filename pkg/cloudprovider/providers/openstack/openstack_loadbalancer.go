...
func (lbaas *LbaasV2) EnsureLoadBalancer(clusterName string, apiService *v1.Service, nodes []*v1.Node) (*v1.LoadBalancerStatus, error) {
	...
	case "false":
		if len(floatingPool) != 0 {
			...
			return nil, fmt.Errorf("floating-network-id or loadbalancer.openstack.org/floating-network-id should be specified when service.beta.kubernetes.io/openstack-load-balancer-internal is false")
		}
	default:
		return nil, fmt.Errorf("unknow service.beta.kubernetes.io/openstack-load-balancer-internal annotation: %v, specify \"true\" or \"false\".",
			internal)
	}
