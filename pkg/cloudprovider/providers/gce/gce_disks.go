...
func (gce *GCECloud) AttachDisk(diskName string, nodeName types.NodeName, readOnly bool) error {
	...
	if gce.AlphaFeatureGate.Enabled(GCEDiskAlphaFeatureGate) {
		...
		if err != nil {
			glog.Warningf("Could not find regional PD named %q to Attach. Will look for a zonal PD", diskName)
			err = nil
		}
	}
...
func (gce *GCECloud) GetAutoLabelsForPD(name string, zone string) (map[string]string, error) {
	...
	case multiZone:
		...
		labels[kubeletapis.LabelZoneFailureDomain] = stringSetToCommaSeparatedString(zoneInfo.replicaZones)
		labels[kubeletapis.LabelZoneRegion] = disk.Region
	case nil:
		// Unexpected, but sanity-check
...
func stringSetToCommaSeparatedString(strSet sets.String) string {
	return strings.Join(strSet.UnsortedList(), ",")
}
