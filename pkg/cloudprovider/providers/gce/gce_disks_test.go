...
type FakeServiceManager struct {
	...
	deleteDiskTargetAPI targetClientAPI
...
func (manager *FakeServiceManager) CreateDisk(
	...
	case *computealpha.Disk:
		...
		manager.diskToCreateAlpha = disk.(*computealpha.Disk)
		manager.disks[zone] = disk.(*computealpha.Disk).Name
		return manager.opAlpha, nil
	case *computebeta.Disk:
		manager.opBeta = &computebeta.Operation{}
		manager.diskToCreateBeta = disk.(*computebeta.Disk)
		manager.disks[zone] = disk.(*computebeta.Disk).Name
		return manager.opBeta, nil
	case *compute.Disk:
		manager.opStable = &compute.Operation{}
		manager.diskToCreateStable = disk.(*compute.Disk)
		manager.disks[zone] = disk.(*compute.Disk).Name
...
func (manager *FakeServiceManager) AttachDisk(
	...
	case *computealpha.AttachedDisk:
		return &computealpha.Operation{}, nil
	case *computebeta.AttachedDisk:
		return &computebeta.Operation{}, nil
...
func (manager *FakeServiceManager) DeleteDisk(
	...
	switch t := manager.deleteDiskTargetAPI; t {
	case targetStable:
		manager.opStable = &compute.Operation{}
		return manager.opStable, nil
	case targetBeta:
