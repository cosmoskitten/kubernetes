...
func (gce *GCECloud) waitForGlobalOp(op APIObject, mc *metricContext) error {
	switch v := op.(type) {
	case *compute_alpha.Operation:
		return gce.waitForOp(alphaOperationToV1(op.(*compute_alpha.Operation)), func(operationName string) (*compute_v1.Operation, error) {
			op, err := gce.serviceAlpha.GlobalOperations.Get(gce.projectID, operationName).Do()
			return alphaOperationToV1(op), err
		}, mc)
	case *compute_beta.Operation:
		return gce.waitForOp(betaOperationToV1(op.(*compute_beta.Operation)), func(operationName string) (*compute_v1.Operation, error) {
			op, err := gce.serviceBeta.GlobalOperations.Get(gce.projectID, operationName).Do()
			return betaOperationToV1(op), err
		}, mc)
	case *compute_v1.Operation:
		return gce.waitForOp(op.(*compute_v1.Operation), func(operationName string) (*compute_v1.Operation, error) {
...
func (gce *GCECloud) waitForRegionOp(op APIObject, region string, mc *metricContext) error {
	switch v := op.(type) {
	case *compute_alpha.Operation:
		return gce.waitForOp(alphaOperationToV1(op.(*compute_alpha.Operation)), func(operationName string) (*compute_v1.Operation, error) {
...
func (gce *GCECloud) waitForZoneOp(op APIObject, zone string, mc *metricContext) error {
	switch v := op.(type) {
	case *compute_alpha.Operation:
		return gce.waitForOp(alphaOperationToV1(op.(*compute_alpha.Operation)), func(operationName string) (*compute_v1.Operation, error) {
