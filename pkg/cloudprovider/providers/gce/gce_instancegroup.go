...
func (gce *GCECloud) CreateInstanceGroup(name string, zone string) (*compute.InstanceGroup, error) {
	...
	obj := compute.InstanceGroup{Name: name}
	glog.V(cloudprovider.APILogLevel).Infof("InstanceGroups.Insert(%s, %s, %v): start", gce.projectID, zone, obj)
	op, err := gce.service.InstanceGroups.Insert(
		gce.projectID, zone, &obj).Do()
	glog.V(cloudprovider.APILogLevel).Infof("InstanceGroups.Insert(%s, %s, %v): end", gce.projectID, zone, obj)
	if err != nil {
		mc.Observe(err)
		return nil, err
	}
...
func (gce *GCECloud) DeleteInstanceGroup(name string, zone string) error {
	...
	glog.V(cloudprovider.APILogLevel).Infof("InstanceGroups.Delete(%s, %s, %s): start", gce.projectID, zone, name)
	op, err := gce.service.InstanceGroups.Delete(
		gce.projectID, zone, name).Do()
	glog.V(cloudprovider.APILogLevel).Infof("InstanceGroups.Delete(%s, %s, %s): end", gce.projectID, zone, name)
	if err != nil {
...
func (gce *GCECloud) ListInstanceGroups(zone string) (*compute.InstanceGroupList, error) {
	...
	glog.V(cloudprovider.APILogLevel).Infof("InstanceGroups.List(%s, %s): start", gce.projectID, zone)
	v, err := gce.service.InstanceGroups.List(gce.projectID, zone).Do()
	glog.V(cloudprovider.APILogLevel).Infof("InstanceGroups.List(%s, %s): end", gce.projectID, zone)
...
func (gce *GCECloud) ListInstancesInInstanceGroup(name string, zone string, state string) (*compute.InstanceGroupsListInstances, error) {
	...
	// TODO: use PageToken to list all not just the first 500
	glog.V(cloudprovider.APILogLevel).Infof("InstanceGroups.ListInstances(%s, %s, %s, %v): start", gce.projectID, zone, name, obj)
	v, err := gce.service.InstanceGroups.ListInstances(
		gce.projectID, zone, name, &obj).Do()
	glog.V(cloudprovider.APILogLevel).Infof("InstanceGroups.ListInstances(%s, %s, %s, %v): end", gce.projectID, zone, name, obj)
	return v, mc.Observe(err)
}
...
func (gce *GCECloud) AddInstancesToInstanceGroup(name string, zone string, instanceRefs []*compute.InstanceReference) error {
	...
	glog.V(cloudprovider.APILogLevel).Infof("InstanceGroups.AddInstances(%s, %s, %s, %v): start", gce.projectID, zone, name, obj)
	op, err := gce.service.InstanceGroups.AddInstances(
		gce.projectID, zone, name, &obj).Do()
	glog.V(cloudprovider.APILogLevel).Infof("InstanceGroups.AddInstances(%s, %s, %s, %v): end", gce.projectID, zone, name, obj)
	if err != nil {
		return mc.Observe(err)
	}
...
func (gce *GCECloud) RemoveInstancesFromInstanceGroup(name string, zone string, instanceRefs []*compute.InstanceReference) error {
	...
	glog.V(cloudprovider.APILogLevel).Infof("InstanceGroups.RemoveInstances(%s, %s, %s, %v): start", gce.projectID, zone, name, obj)
	op, err := gce.service.InstanceGroups.RemoveInstances(
		gce.projectID, zone, name, &obj).Do()
	glog.V(cloudprovider.APILogLevel).Infof("InstanceGroups.RemoveInstances(%s, %s, %s, %v): stop", gce.projectID, zone, name, obj)
	if err != nil {
		return mc.Observe(err)
	}
...
func (gce *GCECloud) SetNamedPortsOfInstanceGroup(igName, zone string, namedPorts []*compute.NamedPort) error {
	...
	glog.V(cloudprovider.APILogLevel).Infof("InstanceGroups.SetNamedPorts(%s, %s, %s, %v): start", gce.projectID, zone, igName, obj)
	op, err := gce.service.InstanceGroups.SetNamedPorts(
		gce.projectID, zone, igName, &obj).Do()
	glog.V(cloudprovider.APILogLevel).Infof("InstanceGroups.SetNamedPorts(%s, %s, %s, %v): stop", gce.projectID, zone, igName, obj)
	if err != nil {
		return mc.Observe(err)
	}
...
func (gce *GCECloud) GetInstanceGroup(name string, zone string) (*compute.InstanceGroup, error) {
	...
	glog.V(cloudprovider.APILogLevel).Infof("InstanceGroups.Get(%s, %s, %s): start", gce.projectID, zone, name)
