...
func (gce *GCECloud) CreateNetworkEndpointGroup(name string, zone string) error {
	if err := gce.alphaFeatureEnabled(AlphaFeatureNetworkEndpointGroup); err != nil {
		return err
	}
	mc := newNetworkEndpointGroupMetricContext("create", zone)
	networkEndpointGroup := &computealpha.NetworkEndpointGroup{
		Name:                name,
		Type:                "LOAD_BALANCING",
		NetworkEndpointType: "GCE_VM_IP_PORT",
		LoadBalancer: &computealpha.NetworkEndpointGroupLbNetworkEndpointGroup{
			Network:    gce.networkURL,
			Subnetwork: gce.subnetworkURL,
		},
	}
	call := gce.serviceAlpha.NetworkEndpointGroups.Insert(gce.GetProjectID(), zone, networkEndpointGroup)
