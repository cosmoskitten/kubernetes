...
func (gce *GCECloud) GetAlphaHealthCheck(name string) (*computealpha.HealthCheck, error) {
	mc := newHealthcheckMetricContext("alpha_get")
	call := gce.serviceAlpha.HealthChecks.Get(gce.projectID, name)
...
func (gce *GCECloud) UpdateAlphaHealthCheck(hc *computealpha.HealthCheck) error {
	mc := newHealthcheckMetricContext("alpha_update")
	call := gce.serviceAlpha.HealthChecks.Update(gce.projectID, hc.Name, hc)
	call.Header().Add(ExperimentalKey, NEGExperimentalValue)
	op, err := call.Do()
	if err != nil {
		return mc.Observe(err)
...
func (gce *GCECloud) CreateAlphaHealthCheck(hc *computealpha.HealthCheck) error {
	mc := newHealthcheckMetricContext("alpha_create")
	call := gce.serviceAlpha.HealthChecks.Insert(gce.projectID, hc)
	call.Header().Add(ExperimentalKey, NEGExperimentalValue)
	op, err := call.Do()
	if err != nil {
		return mc.Observe(err)
