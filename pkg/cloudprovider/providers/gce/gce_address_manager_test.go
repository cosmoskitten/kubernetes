...
func TestAddressManagerBasic(t *testing.T) {
	...
	mgr := newAddressManager(svc, "ABC", region, loadBalancerName, targetIP, schemeInternal)
	err := mgr.HoldAddress()
	if err != nil {
...
func TestAddressManagerOrphaned(t *testing.T) {
	...
	addr := &compute.Address{Name: loadBalancerName, Address: targetIP}
...
func TestAddressManagerOutdatedOrphan(t *testing.T) {
	...
	addr := &compute.Address{Name: loadBalancerName, Address: addrIP}
	if err := svc.ReserveRegionAddress(addr, region); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
