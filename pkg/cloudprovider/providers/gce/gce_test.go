...
func TestGenerateCloudConfigs(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			cloud: func() CloudConfig {
				v := cloudBoilerplate
				v.Network = "my-network"
				v.Subnetwork = "my-subnetwork"
				return v
			},
		},
		{
			...
			cloud: func() CloudConfig {
				v := cloudBoilerplate
				v.Network = "https://www.googleapis.com/compute/v1/projects/project-id/global/networks/my-network"
				v.Subnetwork = "https://www.googleapis.com/compute/v1/projects/project-id/regions/us-central1/subnetworks/my-subnetwork"
				return v
			},
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			genCloud, err := generateCloudConfig(&ConfigFile{Global: tc.config()})
			if err != nil {
				t.Fatalf("Unexpect error: %v", err)
			}

			v := tc.cloud()
			if !reflect.DeepEqual(*genCloud, v) {
				t.Errorf("Expecting cloud config: \n%v,\nbut got\n%v\n", v, *genCloud)
			}
		})
	}
}
