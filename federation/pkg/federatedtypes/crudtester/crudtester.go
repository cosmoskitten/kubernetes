...
func (c *FederatedTypeCRUDTester) CheckLifecycle(desiredObject pkgruntime.Object) {
	...
	c.CheckDelete(obj, &propagationPolicy)
}
...
func (c *FederatedTypeCRUDTester) CheckDelete(obj pkgruntime.Object, propagationPolicy *metav1.DeletionPropagation) {
	namespacedName := c.adapter.NamespacedName(obj)

	c.tl.Logf("Deleting federated %s %q", c.kind, namespacedName)
	err := c.adapter.FedDelete(namespacedName, &metav1.DeleteOptions{PropagationPolicy: propagationPolicy})
	if err != nil {
		c.tl.Fatalf("Error deleting federated %s %q: %v", c.kind, namespacedName, err)
	}

	deletingInCluster := propagationPolicy != nil && *propagationPolicy == metav1.DeletePropagationForeground
