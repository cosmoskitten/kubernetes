...
// If the deletion fails, the enclosing test fails.
func waitForHpaToBeDeletedOrFail(clientset *fedclientset.Clientset, namespace string, hpa string) {
	err := wait.Poll(5*time.Second, fedframework.FederatedDefaultTestTimeout, func() (bool, error) {
...
// If the deletion fails, the enclosing test fails.
func waitForPodToBeDeletedFromClusterOrFail(cluster *fedframework.Cluster, namespace string, podName string) {
	err := wait.Poll(5*time.Second, fedframework.FederatedDefaultTestTimeout, func() (bool, error) {
