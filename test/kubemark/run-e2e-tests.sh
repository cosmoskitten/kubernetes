...
: ${KUBE_ENABLE_CLUSTER_AUTOSCALER:="false"}
if [[ "${KUBE_ENABLE_CLUSTER_AUTOSCALER}" == "true" ]]; then
  KUBEMARK_EXTERNAL_KUBECONFIG=${DEFAULT_KUBECONFIG}
  export KUBEMARK_EXTERNAL_KUBECONFIG
fi
