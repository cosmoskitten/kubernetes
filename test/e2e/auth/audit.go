...
var watchTestTimeout = int64(1)
var auditTestUser = "kubecfg"

type methodCall func()
		...
		testCases := []struct {
		...
		}{
			{
				...
				func() {
					...
					_, err = f.ClientSet.Extensions().Deployments(f.Namespace.Name).Update(d)
					framework.ExpectNoError(err, "failed to update audit-deployment")

					_, err = f.ClientSet.Extensions().Deployments(f.Namespace.Name).Patch(d.Name, types.JSONPatchType, patch)
				...
				func() {
					...
					_, err = f.ClientSet.Core().ConfigMaps(f.Namespace.Name).Patch(configMap.Name, types.JSONPatchType, patch)
				...
				func() {
					...
					_, err = f.ClientSet.Core().Secrets(f.Namespace.Name).Update(secret)
					framework.ExpectNoError(err, "failed to update audit-secret")

					_, err = f.ClientSet.Core().Secrets(f.Namespace.Name).Patch(secret.Name, types.JSONPatchType, patch)
