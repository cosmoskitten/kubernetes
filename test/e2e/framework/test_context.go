...
func RegisterClusterFlags() {
	...
	flag.StringVar(&cloudConfig.ClusterIPRange, "cluster-ip-range", os.Getenv("CLUSTER_IP_RANGE"), "A CIDR notation IP range from which to assign IPs in the cluster.")
...
func AfterReadingAllFlags(t *TestContextType) {
	...
	// Only set the default cluster IP range if it wasn't supplied by CLUSTER_IP_RANGE env var.
	if len(t.CloudConfig.ClusterIPRange) == 0 {
		t.CloudConfig.ClusterIPRange = "10.100.0.0/14"
	}
}
