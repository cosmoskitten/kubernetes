...
var _ = framework.KubeDescribe("Cluster size autoscaler scalability [Slow]", func() {
	...
	It("should scale up at all [Feature:ClusterAutoscalerScalability1]", func() {
		...
		// saturate cluster
		reservationCleanup := ReserveMemory(f, "some-pod", nodeCount*2, nodeCount*perNodeReservation, true, clusterSetUpTimeout)
	...
	It("should scale up twice [Feature:ClusterAutoscalerScalability2]", func() {
		...
		// saturate cluster
		reservationCleanup := ReserveMemory(f, "some-pod", nodeCount, nodeCount*perNodeReservation, true, clusterSetUpTimeout)
		defer reservationCleanup()
		framework.ExpectNoError(waitForAllCaPodsReadyInNamespace(f, c))
