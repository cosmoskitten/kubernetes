...
var _ = SIGDescribe("vsphere statefulset", func() {
	...
	It("vsphere statefulset testing", func() {
		...
		ssPods := statefulsetTester.GetPodList(statefulset)
		Expect(ssPods.Items).NotTo(BeEmpty(), fmt.Sprintf("Unable to get list of Pods from the Statefulset: %v", statefulset.Name))
		Expect(len(ssPods.Items) == int(replicas)).To(BeTrue(), "Number of Pods in the statefulset should match with number of replicas")
		...
		By(fmt.Sprintf("Scaling up statefulsets to number of Replica: %v", replicas+1))
		_, scaleupErr := statefulsetTester.Scale(statefulset, replicas+1)
		Expect(scaleupErr).NotTo(HaveOccurred())
		statefulsetTester.WaitForStatusReplicas(statefulset, replicas+1)
		statefulsetTester.WaitForStatusReadyReplicas(statefulset, replicas+1)
		...
		Expect(len(ssPodsAfterScaleUp.Items) == int(replicas)+1).To(BeTrue(), "Number of Pods in the statefulset should match with number of replicas")
