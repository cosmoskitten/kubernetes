...
var _ = SIGDescribe("vsphere statefulsets", func() {
	...
	It("vsphere statefulsets testing", func() {
		...
		for _, sspod := range ssPods.Items {
			framework.Logf("Waiting for Pod: %q to be Ready", sspod.Name)
			...
			if err != nil {
				...
				for _, volumespec := range sspod.Spec.Volumes {
					if volumespec.PersistentVolumeClaim != nil {
						...
						waitForVSphereDiskToDetach(vsp, vSpherediskPath, types.NodeName(sspod.Spec.NodeName))
					}
				}
			}
		}
		...
		statefulsetsTester.Scale(statefulsets, 4)
		...
		for _, sspod := range ssPodsAfterScaleUp.Items {
			...
			for _, volumespec := range pod.Spec.Volumes {
				if volumespec.PersistentVolumeClaim != nil {
					...
					verifyVSphereDiskAttached(vsp, vSpherediskPath, types.NodeName(sspod.Spec.NodeName))
				}
			}
		}
	})
