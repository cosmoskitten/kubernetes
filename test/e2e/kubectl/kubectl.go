...
var funcMap = template.FuncMap{
	"frontendImage":     imagemanifest.GetfrontendImage,
	"pauseImage":        imagemanifest.GetpauseImage,
	"nginxSlimImage":    imagemanifest.GetnginxSlimImage,
	"nginxSlimNewImage": imagemanifest.GetNewnginxSlimImage,
	"redisImage":        imagemanifest.GetredisImage,
	"redisslaveImage":   imagemanifest.GetredisslaveImage,
	"nautilusImage":     imagemanifest.GetnautilusImage,
	"kittenImage":       imagemanifest.GetkittenImage,
}
...
func substituteImageName(content string) string {
	...
	tmpl, err := template.New("titleTest").Funcs(funcMap).Parse(content)
	if err != nil {
		framework.Failf("Failed Parse the template:", err)
	}
	err = tmpl.Execute(contentWithImageName, nil)
	if err != nil {
...
var _ = SIGDescribe("Kubectl client", func() {
	...
	framework.KubeDescribe("Update Demo", func() {
		...
		BeforeEach(func() {
			...
			nautilus = substituteImageName(string(generated.ReadOrDie(filepath.Join(updateDemoRoot, "nautilus-rc.yaml"))))
			kitten = substituteImageName(string(generated.ReadOrDie(filepath.Join(updateDemoRoot, "kitten-rc.yaml"))))
		})
	...
	framework.KubeDescribe("Guestbook application", func() {
		forEachGBFile := func(run func(s string)) {
			for _, gbAppFile := range []string{
				...
				run(substituteImageName(string(generated.ReadOrDie(gbAppFile))))
			}
		}
