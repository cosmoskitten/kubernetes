...
`), os.FileMode(0755)))
	...
	framework.KubeDescribe("Kubectl reaper", func() {
		...
		It("should delete statefulset and all of the resources managed by it", func() {
			...
			// We also need to delete the related service
			By("deleting the service related to statefulset " + name)
			framework.RunKubectlOrDie("delete", "service", name, nsFlag, "--cascade=true")

			err = wait.PollImmediate(time.Second, time.Minute, func() (bool, error) {
				_, err := c.Core().Services(ns).Get(name, metav1.GetOptions{})
				if apierrs.IsNotFound(err) {
					return true, nil
				} else if err != nil {
					return false, err
			...
			if err != nil {
				framework.Failf("Failed verifying statefulset's service was deleted: %v", err)
			}
		})
	})
})
