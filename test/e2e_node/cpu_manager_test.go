...
func setOldKubeletConfig(f *framework.Framework) {
	if oldCfg != nil {
		framework.ExpectNoError(setKubeletConfiguration(f, oldCfg))
	}
}

func setupForTests(f *framework.Framework) {
	...
	newCfg.CPUManagerPolicy = cpuManagerPolicy

	// Set the CPU Manager reconcile frequency to 1 second.
	newCfg.CPUManagerReconcileFrequency = metav1.Duration{Duration: 1 * time.Second}
