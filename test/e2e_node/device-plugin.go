...
var _ = framework.KubeDescribe("[Feature:GPUDevicePlugin]", func() {
	...
	BeforeEach(func() {
		...
		if framework.NumberOfGPUs(getLocalNode(f)) == 0 {
			return
		}
		...
		Eventually(func() bool { return framework.NumberOfGPUs(getLocalNode(f)) > 0 },
			driverInstallTimeout*2, time.Second).Should(BeTrue())
	})

	// Cleanup pods
	AfterEach(func() {
		pods, err := f.PodClient().List(metav1.ListOptions{})
		...
		for _, p := range pods.Items {
			...
			framework.ExpectNoError(f.PodClient().Delete(p.Name, &metav1.DeleteOptions{}))
		}
	})
	...
	It("checks that when Kubelet restarts exclusive GPU assignation to pods is kept.", func() {
		...
		if framework.NumberOfGPUs(getLocalNode(f)) < 2 {
