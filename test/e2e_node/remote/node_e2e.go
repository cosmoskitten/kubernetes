...
// prependGCIMounterFlag prepends the flag for setting the GCI mounter path to
// args and returns the result.
func prependGCIMounterFlag(args, host, workspace string) (string, error) {
...
func updateOSSpecificKubeletFlags(args, host, workspace string) (string, error) {
	...
	case strings.Contains(output, "ID=gci"), strings.Contains(output, "ID=cos"):
		...
		return prependGCIMounterFlag(args, host, workspace)
