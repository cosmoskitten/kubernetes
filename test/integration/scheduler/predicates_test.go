...
// anti-affinity predicates function works correctly.
func TestInterPodAffinity(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			node: nodes[0],
			fits: true,
			test: "validates that InterPodAffinity is respected if matching with multiple Affinitie in multiple RequiredDuringSchedulingIgnoredDuringExecution ",
		},
		{
			pod: &v1.Pod{
				ObjectMeta: metav1.ObjectMeta{
		...
		},
		{
			...
			fits: false,
			test: "nodes[0] and nodes[1] have same topologyKey and label value. Node2 has an existing pod that match the inter pod affinity rule. The pod can not be scheduled onto either of the two nodes.",
		},
	}
