...
func TestFederationCRUD(t *testing.T) {
	...
	// Validate deletion handling when PropagationPolicy is "Orphan"
	propagationPolicy := metav1.DeletePropagationOrphan
	testCases := map[string]*metav1.DeletionPropagation{
		"Resource should not be deleted from underlying clusters when defaultPropagationPolicy is Orphan": &propagationPolicy,
	}

	for testName, propagationPolicy := range testCases {
		t.Run(testName, func(t *testing.T) {
			fixture, crudTester, obj, _ := initCRUDTest(t, &fedFixture, adapterFactory, kind)
			defer fixture.TearDown(t)

			updatedObj := crudTester.CheckCreate(obj)
			crudTester.CheckDelete(updatedObj, propagationPolicy)
		})
	}
